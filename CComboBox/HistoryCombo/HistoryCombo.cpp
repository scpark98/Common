...
BEGIN_MESSAGE_MAP(CHistoryCombo, CComboBox)
	...
	//}}AFX_MSG_MAP
	ON_CONTROL_REFLECT(CBN_KILLFOCUS, &CHistoryCombo::OnCbnKillfocus)
	ON_CONTROL_REFLECT(CBN_SETFOCUS, &CHistoryCombo::OnCbnSetfocus)
END_MESSAGE_MAP()
...
void CHistoryCombo::OnCbnKillfocus()
{
	CString str;
	
	GetWindowText(str);
	if (str.IsEmpty())
		SetWindowText(_T("kill focus"));
}


void CHistoryCombo::OnCbnSetfocus()
{
	CString str;

	GetWindowText(str);
	if (str == _T("kill focus"))
		SetWindowText(_T(""));
}

void CHistoryCombo::GetWindowText(CString& text)
{
	CString str;
	CComboBox::GetWindowText(str);

	if (m_placeholder.IsEmpty() == false && str == m_placeholder)
	{
		text.Empty();
	}
	else
