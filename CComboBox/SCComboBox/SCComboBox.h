...
//입력 시 동적 필터링 기능
	//listbox를 펼쳐놓은 상태에서는 자동 필터링된다.
	//그렇다면 타이핑이 시작되면 listbox를 자동으로 펼쳐줘야 한다.
	//일단 기본값은 true로 한다.
	...
	// WM_IME_COMPOSITION + CBN_EDITCHANGE 가 동일 상태에서 연속 호출되는 경우
	// listbox 재구성을 1번만 수행하기 위한 이전 filter 캐시.
	CString			m_last_filter;

	// 지연 필터링용 타이머. 입력 이벤트마다 타이머를 reset 해 "멈춤 시점" 을 감지.
	// 타이머 만료 시 composing 이면 IMM 에 CPS_COMPLETE 로 강제 commit 해 CBN_EDITCHANGE 가
	// non-composing 상태로 재진입. delay 값은 SCComboBox.cpp 의 상수로 정의.
