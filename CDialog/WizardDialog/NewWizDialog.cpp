...
BOOL CNewWizDialog::ActivatePage(CNewWizPage* pPage)
{
  ...
  ScreenToClient(&rect);
  pPage->SetWindowPos(NULL, rect.left, rect.top, rect.Width(), rect.Height(),
...
BOOL CNewWizDialog::OnInitDialog() 
{
	...
	//ModifyStyleEx (0, WS_EX_CONTROLPARENT);	// only for white background dialog
...
BOOL CNewWizDialog::SetActivePageByResource(UINT nResourceID)
{
...
void CNewWizDialog::SetPreviousPage()
{
	//OnWizardBack();
...
void CNewWizDialog::FinishWizard()
{
//	CDialog::OnCancel();
	OnWizardFinish();
}

BOOL CNewWizDialog::PreTranslateMessage(MSG* pMsg) 
{
