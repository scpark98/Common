...
public:
    ...
    enum MESSAGES
    {
        ...
        message_scstaticedit_action_button, // action 버튼 클릭. parent 가 처리해야 하는 action 일 때 발신.
                                            // 어떤 action 인지는 get_action_button() 으로 확인.
    };

    // 우측에 표시되는 action 버튼 종류. action_none 이면 미표시.
    //   - action_copy             : 현재 텍스트를 클립보드로 복사. 자체 처리(parent 알림 X).
    ...
    //Transparent(기본) = 기존 기본(text=테마 cr_text_disabled 값, back=LightGray) 사용, 불투명색 = 그 색.
    //readonly 와 달리 disabled 는 enabled 상태와 시각 구분이 필요하므로 테마색으로 override 하는 용도.
    ...
    //   - margin.right 만큼 action 버튼 / text 우측이 좌측으로 밀림.
    //   - margin.top / margin.bottom 만큼 세로 영역 축소.
    void		    set_margin(CRect margin);
    CRect		    get_margin() const { return m_margin; }

    // 우측에 action 버튼 표시. action_none 이 아니면 텍스트 영역이 우측으로 줄어들고 그 자리에
    // action 별 아이콘을 그린다. 별도 CButton 없이 OnPaint/마우스 핸들러에서 직접 처리.
    // action_password_toggle 호출 시 password 모드도 자동 ON (mask 기본 '*').
    ...
    //   텍스트 영역은 좌측으로 (2 + 계산된 width + 5)px 줄어든다.
    bool                    set_prefix_image(UINT id, int size = 16);
    void                    clear_prefix_image();
    bool                    has_prefix_image() const { return m_prefix_image.m_pBitmap != nullptr; }
...
private:
    ...
    //20260728 by claude. disabled 본문 텍스트색 사용자 override. set_text_color_disabled 가 호출되면 m_text_color_disabled_user_set=true,
    //그 색을 m_cr_text_disabled_override 에 보관. 미호출(기본)이면 시스템 COLOR_GRAYTEXT(네이티브 CSCEdit 과 동일).
    //m_theme.cr_text_disabled(테마 전파값)와 별개로 보관 — 전파(copy_colors_from)가 사용자 지정색을 덮어쓰지 않게 하기 위함.
    ...
    // ── Action 버튼 (우측 아이콘) ──
    action_button_type	m_action_type             = action_none;
    bool				m_action_button_capturing = false; // 마우스 캡처 중
    bool				m_action_button_pressed   = false; // 시각 상태 (캡처 중 + 영역 내)
    CRect				m_r_action_button;                 // 버튼 그려지는 최대 영역. draw_action_button 에서 갱신, hit-test 에서 사용.
    void				draw_action_button(Gdiplus::Graphics& g);
    void				perform_action_button_click();      // m_action_type 별 click 동작 분기

    // 개별 action 그리기. m_r_action_button 을 직접 참조. 눌림 offset 은 호출 측 Graphics translate 로 일괄 처리.
    void				draw_action_icon_copy           (Gdiplus::Graphics& g);
    ...
    int					m_prefix_image_size = 16; // 이미지 그릴 높이(px). 항상 이 값으로 그림.
    CRect				get_prefix_image_area() const;
    void				draw_prefix_image(Gdiplus::Graphics& g);
