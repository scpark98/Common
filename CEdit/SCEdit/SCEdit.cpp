...
bool CSCEdit::create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	//CEdit과는 달리 CMaskedEdit에서는 Shift+Home/End로 범위 선택이 불가하다.
...
void CSCEdit::OnNcPaint()
{
	...
	if (/*m_transparent && */m_draw_border)
	{
		...
		CPen pen(PS_SOLID, m_border_width, (GetFocus() == this) ? m_theme.cr_border_active.ToCOLORREF() : m_theme.cr_border_inactive.ToCOLORREF());
...
BOOL CSCEdit::OnEraseBkgnd(CDC* pDC)
{
	...
	Gdiplus::Color cr_border = (GetFocus() == this) ? m_theme.cr_border_active : m_theme.cr_border_inactive;
...
void CSCEdit::GetWindowText(CString& text) const
{
	...
	int nLineCount = (int)::SendMessage(m_hWnd, EM_GETLINECOUNT, 0, 0);
	for (int i = 0; i < nLineCount; i++)
	{
		int nLen = (int)::SendMessage(m_hWnd, EM_LINELENGTH,
			::SendMessage(m_hWnd, EM_LINEINDEX, i, 0), 0);
		CString line;
		if (nLen > 0)
		{
			LPTSTR pBuf = line.GetBufferSetLength(nLen + 1);
			*((LPWORD)pBuf) = (WORD)(nLen + 1);  // EM_GETLINE에 필요한 버퍼 크기 지정
			int nRead = (int)::SendMessage(m_hWnd, EM_GETLINE, i, (LPARAM)pBuf);
			line.ReleaseBuffer(nRead);
		}
		if (i > 0)
			text += _T("\r\n");
		text += line;
...
void CSCEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (bShift && (nChar == VK_HOME || nChar == VK_END))
	{
		DWORD dwStart = 0;
		DWORD dwEnd = 0;
		SendMessage(EM_GETSEL, reinterpret_cast<WPARAM>(&dwStart), reinterpret_cast<LPARAM>(&dwEnd));

		const int nTextLen = GetWindowTextLength();

		// 선택 anchor:
		// 일반 CEdit처럼 "선택이 없으면 caret 위치를 anchor로 사용"
		// 이미 선택 중이면 반대편 끝을 anchor로 유지
		int nAnchor = 0;
		int nCaret = 0;

		if (dwStart == dwEnd)
		{
			nAnchor = static_cast<int>(dwEnd);
			nCaret = static_cast<int>(dwEnd);
		}
		else
		{
			// caret이 어느 쪽 끝에 있는지 완벽히 알 수는 없으므로
			// Shift+방향키 연장과 최대한 비슷하게:
			// Home이면 오른쪽 끝을 anchor로 유지
			// End이면 왼쪽 끝을 anchor로 유지
			if (nChar == VK_HOME)
			{
				nAnchor = static_cast<int>(dwEnd);
				nCaret = static_cast<int>(dwStart);
			}
			else // VK_END
			{
				nAnchor = static_cast<int>(dwStart);
				nCaret = static_cast<int>(dwEnd);
			}
		}

		int nNewCaret = (nChar == VK_HOME) ? 0 : nTextLen;

		// EM_SETSEL은 start/end 순서와 무관하게 selection 설정 가능
		SendMessage(EM_SETSEL, static_cast<WPARAM>(nAnchor), static_cast<LPARAM>(nNewCaret));
		return;
	}
