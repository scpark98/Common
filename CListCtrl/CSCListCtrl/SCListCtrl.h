...
//기타 레이아웃 관련
	...
	//격자선(gridlines) 표시 여부. 활성 시 각 셀의 우측/하단에 1px 라인을 그린다(가로/세로 격자).
	//20260723 by claude. on/off 상태는 별도 멤버가 아니라 LVS_EX_GRIDLINES 비트에 보관한다.
	//이 컨트롤은 owner-draw 라 native 가 격자를 그려주지는 않지만, 비트를 단일 소스로 쓰면
	//SetExtendedStyle(...|LVS_EX_GRIDLINES) / 리소스 편집기의 Gridlines 속성 / 이 함수 — 세 경로가 모두 같은 상태를 가리킨다.
	//cr_grid 가 Transparent(기본)면 색은 건드리지 않는다 — 테마가 cr_back 에서 파생한 m_theme.cr_gridlines 을 그대로 쓴다.
	void		use_gridlines(bool use, Gdiplus::Color cr_grid = Gdiplus::Color::Transparent)
	{
		DWORD ex = GetExtendedStyle();
		SetExtendedStyle(use ? (ex | LVS_EX_GRIDLINES) : (ex & ~LVS_EX_GRIDLINES));
		if (cr_grid.GetValue() != Gdiplus::Color::Transparent)
			m_theme.cr_gridlines = cr_grid;
		Invalidate();
	}
	bool		is_use_gridlines() const { return (GetExtendedStyle() & LVS_EX_GRIDLINES) != 0; }

	//20260831 by claude. 셀 텍스트를 화면에서 온전히 읽을 수 없을 때, 그 셀에 마우스를 올리면
	//전체 내용을 툴팁으로 보여준다(탐색기와 같은 동작). 온전히 보이는 셀에는 뜨지 않는다.
	//
	//20260901 by claude. [판정 기준 — CSCTreeCtrl 과 동일]
	//  "가려져서 못 읽으면 뜬다." 두 경우를 모두 포함한다.
	//   (1) 컬럼이 좁아 "..." 로 줄어든 경우
	//   (2) 컬럼은 넓은데 컨트롤이 좁아(또는 가로 스크롤로) 셀 일부가 화면 밖으로 나간 경우
	//  즉 비교 대상은 컬럼 폭이 아니라 *화면에 보이는 폭* 이다(draw_row 의 판정 주석 참조).
	//
