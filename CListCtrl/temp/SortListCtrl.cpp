...
void CSortListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	for ( int i = 0; i < m_iNumColumns; i++ )
	{
		GetSubItemRect( nIndex, i, LVIR_BOUNDS, Rect );

		if ( GetItemState(nIndex, LVIS_SELECTED) )
			pDC->FillSolidRect( Rect, RGB(255,234,176) );
		else
			pDC->FillSolidRect( Rect, RGB(255,250,236) );

		Rect.left	+= 2;
		Rect.top	+= 1;
		sItemText = GetItemText( nIndex, i );

		pDC->DrawText( sItemText, Rect, DT_LEFT );
	}
}
...
int CSortListCtrl::GetSelectedItem()
{
