...
static  LOGFONT g_lf_user = {-14, 0, 0, 0, FW_NORMAL,
							  0, 0, 0, DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
							  CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
							  DEFAULT_PITCH,_T("맑은 고딕") };
#endif
	...
	m_hAccelToDraw((HACCEL)INVALID_HANDLE_VALUE)
{
	...
	{
		m_lf_height	= -14;
		g_lf_user.lfHeight	= m_lf_height;
	}
...
//scpark
int			CNewMenu::m_color_theme = -1;

COLORREF	CNewMenu::m_cr_back;
COLORREF	CNewMenu::m_cr_back_hilight;

COLORREF	CNewMenu::m_cr_text;
COLORREF	CNewMenu::m_cr_text_gray;
COLORREF	CNewMenu::m_cr_text_hilight;

COLORREF	CNewMenu::m_cr_separator;
COLORREF	CNewMenu::m_cr_border;
COLORREF	CNewMenu::m_cr_check;

int			CNewMenu::m_lf_height = 0;
int			CNewMenu::m_line_height = 0;
int			CNewMenu::m_menu_min_width = 250;

void CNewMenu::set_color_theme(int theme)
{
	if (theme == color_theme_darkgray)
	{
		m_color_theme		= theme;

		m_cr_back			= RGB(27, 27, 28);
		m_cr_back_hilight	= RGB(51, 51, 52);

		m_cr_text			= RGB(192, 192, 192);
		m_cr_text_gray		= RGB(101, 101, 101);
		m_cr_text_hilight	= RGB(192, 212, 242);;

		m_cr_separator		= RGB(51, 51, 55);
		m_cr_border			= 0;
		m_cr_check			= m_cr_text;
	}
	else if (theme == color_theme_daumpot)
	{
		m_color_theme		= theme;

		m_cr_back			= RGB(68, 68, 68);
		m_cr_back_hilight	= RGB(32, 32, 32);//DarkenColor(60, m_cr_back);

		m_cr_text			= RGB(225, 225, 225);//RGB(187, 187, 187);
		m_cr_text_gray		= RGB(119, 119, 119);
		m_cr_text_hilight	= RGB(225, 188, 87);

		m_cr_separator		= RGB(16, 16, 16);
		m_cr_border			= 0;
		m_cr_check			= m_cr_text;
	}
	else// if (theme == color_theme_default)
	{
...
void CNewMenu::set_lf_height(int height)
{
	m_lf_height = height;
	g_lf_user.lfHeight	= m_lf_height;
}

void CNewMenu::set_line_height(int line_height)
{
...
void CNewMenu::GetMenuBarColor2003(COLORREF& color1, COLORREF& color2, BOOL bBackgroundColor /* = TRUE */)
{
		...
		GetMenuDrawMode() == STYLE_COLORFUL)
	{
		...
		// some colorscheme corrections (Andreas Sch?er)
		...
		else if (nearColor == 13425878) //olivgr?
		{
...
void CNewMenu::DrawItem_XP_2003(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	COLORREF colorWindow = m_cr_back;//DarkenColor(10, GetSysColor(COLOR_WINDOW));
	COLORREF colorMenuBar = bIsMenuBar ? GetMenuBarColor() : GetMenuColor();
	COLORREF colorMenu = m_cr_back;//RGB(255,0,0);//MixedColor(colorWindow, colorMenuBar);
	COLORREF colorBitmap = MixedColor(GetMenuBarColor(), colorWindow);
	COLORREF colorSel = GetXpHighlightColor();
	COLORREF colorBorder = RGB(255,0,0);//GetSysColor(COLOR_HIGHLIGHT);//DarkenColor(128,colorMenuBar);
	...
	else
	{
		...
		else
		{
			...
			pDC->FillSolidRect(RectIcon, colorMenu);
		}
		// Draw for Textbackground
		pDC->FillSolidRect(RectText, colorMenu);
	}
...
void CNewMenu::DrawItem_Icy(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	COLORREF colorMenu = RGB(255, 128, 128);//GetMenuColor();
	COLORREF colorBack = RGB(255, 128, 128);//bIsMenuBar?GetMenuBarColor():colorMenu;
...
void CNewMenu::DrawItem_OldStyle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	if (nFlags & MF_SEPARATOR)
	{
		...
		else
		{
			rect = lpDIS->rcItem;
			//scpark
			pDC->FillSolidRect(rect, m_cr_separator);
			rect.left += 1;
			rect.top += 1;
			//scpark
			//pDC->DrawEdge(&rect, EDGE_ETCHED, BF_TOP);
	...
	else
	{
		...
		// set some colors and the font
		//scpark
		m_brSelect.CreateSolidBrush(m_cr_back_hilight);
		...
		{
			// Draw the background
			pDC->FillSolidRect(rect, m_cr_back);
		}
		...
		if (state&ODS_SELECTED)
		{
			...
			if (!bIsMenuBar)
			{
				...
				{
					//scpark
					//체크박스 등이 있어도 선택 항목의 배경색은 매뉴바 너비 그대로 그리자.
					//rect2 = RectText;
				}
		...
		if (!strText.IsEmpty())
		{
				...
				!(lpDIS->itemState & (ODS_DISABLED)))
			{
				...
				else
				{
					...
					{
						DrawShadowText(pDC->GetSafeHdc(), leftStr, leftStr.GetLength(),
										&RectText, nFormat,
										crText, 0, 2, 1);
					}
					...
					if (tablocr != -1)
					{
						...
						{
							DrawShadowText(pDC->GetSafeHdc(), rightStr, rightStr.GetLength(),
											&RectText, nFormatr,
											crText, 0, 2, 1);
						}
					}
			...
			else
			{
				...
				if (!(state & ODS_SELECTED))
				{
					...
					pDC->SetTextColor(m_cr_text_gray);
...
void CNewMenu::MeasureItem_OldStyle(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar)
{
	...
	else
	{
		...
		{
			int width = m_iconX + size.cx + m_iconX + GAP;
			//scpark
			//minimum width = 300;
			lpMIS->itemWidth = (width > m_menu_min_width ? width : m_menu_min_width);
		}

		int temp = GetSystemMetrics(SM_CYMENU);

		//scpark
		lpMIS->itemHeight = m_line_height;//(temp > (m_iconY + 4)) ? temp : (m_iconY + 4);
...
void CNewMenu::MeasureItem_WinXP(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar)
{
	...
	else
	{
		...
		else
		{
			...
			//scpark
			lpMIS->itemHeight = m_line_height;//temp > m_iconY + 8 ? temp : m_iconY + 7;
		}
...
void CNewMenu::DrawSpecialCharStyle(CDC* pDC, LPCRECT pRect, TCHAR Sign, DWORD dwStyle)
{
	...
	if ((dwStyle&ODS_GRAYED) || (dwStyle&ODS_INACTIVE))
	{
		oldColor = pDC->SetTextColor(m_cr_text_gray);//::GetSysColor(COLOR_GRAYTEXT));
	}
	else
	{
		oldColor = pDC->SetTextColor(m_cr_check);//::GetSysColor(COLOR_MENUTEXT));
	}
...
void CNewMenu::DrawSpecialChar(CDC* pDC, LPCRECT pRect, TCHAR Sign, BOOL bBold)
{
	...
	pDC->DrawText(&Sign, 1, rect, DT_CENTER | DT_SINGLELINE | DT_VCENTER);
...
{
	//scpark 
	//pDC->FillSolidRect(rectInner, RGB(255,255,0));
	//rectOuter.InflateRect(3, 3);
	//pDC->FillSolidRect(rectOuter, RGB(255,68,68));
	//return;
...
void CMenuTheme::GetBarColor(COLORREF &clrUpperColor, COLORREF &clrMediumColor, COLORREF &clrBottomColor, COLORREF &clrDarkLine)
{
	if (IsMenuThemeActive())
	{
		...
		// corrections from Andreas Sch?er
		switch (menuColor)
		{
	...
	if (pData->m_bDoSubclass)
	{
		...
		else
		{
			...
			if (g_Shell >= Win2000 && CNewMenu::GetAlpha() != 255)
			{
				...
				SetLayeredWindowAttributes(hWnd, 0, CNewMenu::GetAlpha(), LWA_ALPHA);
				pData->m_dwData |= 4;
				SetTimer(hWnd, MENU_TIMER_ID, 10, NULL);
			...
			//scpark
			//shading을 제거하는 코드. OnUnInitWnd()에 복원시키는 코드도 역시 필요하다.
...
BOOL CMenuTheme::OnUnInitWnd(HWND hWnd)
{
	...
	if (pData)
	{
		...
		//scpark
		//shading을 복원하는 코드. OnInitWnd()에 shading을 제거하는 코드가 존재한다.
...
void CNewMenuHelper::set_color_theme(CNewMenu::COLOR_THEME theme)
{
	CNewMenu::set_color_theme(theme);
}

void CNewMenuHelper::set_lf_height(int height)
{
	CNewMenu::set_lf_height(height);
}

void CNewMenuHelper::set_line_height(int line_height)
{
	CNewMenu::set_line_height(line_height);
}
