...
void CCustomTreeChildCtrl::SetTreeButtonImage( UINT nButtonID )
{
...
void CCustomTreeChildCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (CheckHit(point))
	{
		...
		if(hItem)
		{
			...
			HTREEITEM ht = HitTest(point, &uFlags);

			/*
			if( uFlags & LVHT_ONITEMLABEL ) // choices are: LVHT_ONITEMICON,
				//LVHT_ONITEMLABEL,
				// LVHT_ONITEMSTATEICON, LVHT_ONITEM,
				
			{
				// here do your thing for clicking on the checkbox
				TRACE(_T("you clicked on the state icon...\n"));
			}
			else if ( uFlags & LVHT_ONITEM )
			{ // another action for on the item
				TRACE(_T("you clicked on the item...\n"));
			}
			*/

			//check tree인 경우 checkbox만 클릭해서 토글할지
			//fullrowselection일 경우 선택만 해도 토글할지를 나중에는 옵션처리해야 한다.
			//항목만 선택해도 체크가 토글되도록 하려는데
			//체크박스를 클릭하면 토글이 중복된다. 우선 편법으로 체크가 아닌 영역이 눌리면 토글되도록 함.
			//CRect rcItem;
			if (GetParentItem(hItem) != NULL)	//sub항목만 체크박스. 나중 처리 필요.
			{
				GetItemRect(hItem, &rcItem, false);
				rcItem.left += 40;
				if (rcItem.PtInRect(point))
				{
					...
					GetParent()->GetParent()->SendMessage(MSG_COLUMNTREECTRL_CHECKED_CHANGED, (WPARAM)hItem, (LPARAM)GetCheck(hItem));
				}
				else
				{
					//checkbox를 눌러서 메시지를 전송하면 한박자 늦은 상태값이 전달된다.
					//우선 반대값을 전달해야 현재 체크상태가 올바로 전달된다.
					GetParent()->GetParent()->SendMessage(MSG_COLUMNTREECTRL_CHECKED_CHANGED, (WPARAM)hItem, (LPARAM)(!GetCheck(hItem)));
				}
			}
		}
...
LRESULT CCustomTreeChildCtrl::OwnerDraw(CDC* pDC)
{
	...
	CSize iconSize(10,10), stateImgSize;
	...
	if (bmpMem.CreateCompatibleBitmap(&dc, rcClient.Width(), rcClient.Height()))
	{
		...
		{		
			...
			if ((GetItemState(hItem, selflag) & selflag) 
				&& ::GetFocus() == m_hWnd)
				;
			else if (true)//m_mapColorFont.Lookup(hItem, cf))
			{
				...
				if(false)//cf.logfont.lfFaceName[0] != '\0') 
					;//logfont = cf.logfont;
				else {
				...
				pFontDC = dcMem.SelectObject(&fontDC );
				/*
				...
				*/
				...
				dcMem.TextOut(rect.left + 2, rect.top + 1, sItem);

				dcMem.SelectObject(pFontDC);
			}
			hItem = GetNextVisibleItem(hItem);
		}
#endif
...
void CCustomTreeChildCtrl::LoadSaveTreeTabData( BOOL bLoad, CString sFileName, bool checkAtRoot )
{
	FILE*		fp;

	if ( !bLoad )
	{
		if ( (fp = _tfopen( sFileName, _T("w") )) == NULL )
			return;
		...
		while( hti )
		{
			...
			_ftprintf( fp, _T("%s\r"), GetItemText( hti ) );
	...
	else
	{
		...
		if ( _fgetts( chLine, 1000, fp ) == NULL )
			return;
		...
		do
		{
			...
			hti = InsertItem( sLine, 1, 0, parent ? parent : TVI_ROOT, TVI_LAST );
			if (indent == 0 && !checkAtRoot)
				SetItemState(hti, INDEXTOSTATEIMAGEMASK(0), TVIS_STATEIMAGEMASK); 
...
int CCustomTreeChildCtrl::GetIndentLevel( HTREEITEM hItem )
{
	int iIndent = 0;
...
#if 0
HTREEITEM CCustomTreeChildCtrl::FindData( HTREEITEM hti, DWORD dwData, BOOL bAutoSelect /*=TRUE*/ )
{
	if( hti == NULL )
		hti = GetRootItem();
	...
	do
	{
		...
		if( ( hti_res = FindData( hti, dwData, bAutoSelect ) ) != NULL )
			return hti_res;
...
HTREEITEM CCustomTreeChildCtrl::FindData( HTREEITEM hti, CString sData, BOOL bAutoSelect /*=TRUE*/ )
{
	if( hti == NULL )
		hti = GetRootItem();

	CString itemText = GetItemText( hti );
	TRACE(_T("%s\n"), itemText);
	if ( itemText == sData )
	{
		if ( bAutoSelect )
	...
	do
	{
		...
		if( ( hti_res = FindData( hti, sData, bAutoSelect ) ) != NULL )
			return hti_res;
...
// 자식노드중에 해당 문자열 항목을 찾는다. 비재귀호출
HTREEITEM CCustomTreeChildCtrl::FindChildItem( HTREEITEM hti, CString sString )
{
	hti = GetChildItem( hti );

	do
	{
		if ( GetItemText( hti ) == sString )
			return hti;
	} while( ( hti = GetNextSiblingItem( hti ) ) != NULL );
...
/*
void CCustomTreeChildCtrl::SetCheckAll(bool bCheck)
{
	CString str;
	Traverse(GetRootItem());
}
void CCustomTreeChildCtrl::Traverse(HTREEITEM hItem)
{
	HTREEITEM hItemSub = NULL;
	TRACE("1:%s\n", GetItemText(hItem));
	if (GetItemText(hItem) == _T("친구"))
		return;
...
*/
#if 0
HTREEITEM CCustomTreeChildCtrl::Traverse(/*CString ItemText, */HTREEITEM hti)
{
	CString text;
	if(hti == NULL)
	{
		return NULL; //set break point here
	}

	hti = GetChildItem( hti );
	TRACE("1:%s\n", GetItemText(hti));
	if(false)// GetItemText(hti) == ItemText )
	{
		return hti; //set break point here
	}

	do
	{
		Traverse( /*ItemText, */hti);
		TRACE("2:%s\n", GetItemText(hti));
		if (false)//GetItemText(hti) == ItemText)
		{
			return hti; //set break point here. This should ensure a proper return.
		}
	}while( (hti = GetNextSiblingItem( hti )) != NULL );
...
#endif
#if 0
void CCustomTreeChildCtrl::Iterate(HTREEITEM hItem)
{
	HTREEITEM hLastValid;

	if (hItem)
	{
		// Use the tree node corresponding to hItem
		// End of using hItem
		hItem = CTreeCtrl::GetNextItem(hItem, TVGN_CHILD);
		while (hItem)
		{
			Iterate(hItem);
			hItem = CTreeCtrl::GetNextItem(hItem, TVGN_NEXT);
			...
			if (hItem == NULL)
			{
				...
				if (hLastValid)
				{
					hItem = CTreeCtrl::GetNextItem(GetParentItem(hLastValid), TVGN_NEXT);
					if (hItem == NULL)
					{
						hLastValid = NULL;
						//break;
	...
	{
		HTREEITEM hItem = CTreeCtrl::GetNextItem(NULL, TVGN_ROOT);
		while (hItem)
		{
			Iterate(hItem);
			hItem = CTreeCtrl::GetNextItem(hItem, TVGN_NEXT);
...
//https://mariusbancila.ro/blog/2007/06/29/finding-an-item-in-a-ctreectrl/
...
HTREEITEM CCustomTreeChildCtrl::FindItem(const CString& findText, int check)
{
	HTREEITEM root = GetRootItem();
	while(root != NULL)
	{
		HTREEITEM hFound = FindItem(root, findText, check);
		if (hFound)
			return hFound; 
...
//findText가 ""이면 모든 노드를 순회해서 check가 0또는 1이라면 체크 변경.
//findText가 ""이 아니면 해당 아이템만 찾아서 check.
//단, check가 0또는 1이 아니라면 단순히 findText의 아이템을 찾고자 하는 목적으로 이용된다.
HTREEITEM CCustomTreeChildCtrl::FindItem(HTREEITEM hRoot, const CString& findText, int check )
{
	...
	{	
		if (check == 0 || check == 1)
		{
			if (GetParentItem(hRoot) != NULL)	//최상위 레벨은 체크 표시를 하지 않기 위해.
				SetCheck(hRoot, check);
		}
	}
	else
	{
		if (GetItemText(hRoot) == findText)
		{
			if (check == 0 || check == 1)
				SetCheck(hRoot, check);
			return hRoot; 
	...
	while (hSub)
	{
		...
		HTREEITEM hFound = FindItem(hSub, findText, check);
		if (hFound)
			return hFound; 
