...
BOOL CSCTreeCtrl::OnTvnBegindrag(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	::SetFocus(m_hWnd);
	//20260712 by claude. 드래그 시작 시 드래그 항목을 선택하지 않는다 — 기존 선택(트리 하이라이트 + 리스트)이 그대로
	//유지되어야 하기 때문(사용자 요구). 드래그 표시는 드래그 이미지(m_drag_shape)로만. 또 up 의 지연 선택 후보도 취소.
...
void CSCTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (hItem)
	{
		...
		if (!rcButton.IsRectEmpty() && rcButton.PtInRect(point))
		{
			...
			//이 stale 상태에선 대상 노드를 refresh(자식 재열거)한다 — 노드가 이미 expanded 라 재삽입된 자식이 바로 표시된다.
			if ((GetItemState(hItem, TVIS_EXPANDED) & TVIS_EXPANDED) && GetChildItem(hItem) == NULL)
				refresh(hItem);
			else
	...
	if (hItem)
	{
		...
		//base 리턴 직후(아래)에서. 드래그 소스는 OnTvnBegindrag 이 클릭 지점 항목으로 독립적으로 잡으므로 안전.
