...
//자체 overlay scrollbar — native WS_VSCROLL / WS_HSCROLL 제거 + 우측 / 하단 child 로 그림.
	...
	//20260712 by claude. 리사이즈 중 '바 숨김' 최적화 적용 여부. 기본 false(바 유지). 정말 느린 앱만 set_hide_scroll_when_resize(true).
...
public:
	//라벨을 화면에서 온전히 읽을 수 없을 때 hover 툴팁을 띄울지 여부. 기본 켜짐.
	//20260901 by claude. [판정 기준 — CSCListCtrl 과 동일]
	//  "가려져서 못 읽으면 뜬다." 라벨 rect 를 화면에 보이는 오른쪽 끝으로 자른 뒤 글자 폭과 비교한다.
	//  트리가 좁아서든 들여쓰기가 깊어서든, 결과적으로 안 보이면 뜬다.
