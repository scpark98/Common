...
#define CTREECTRLEXWND_CLASSNAME _T("CTreeCtrlNodeEx")

// CTreeCtrlNodeEx

IMPLEMENT_DYNAMIC(CTreeCtrlNodeEx, CTreeCtrl)
...
BEGIN_MESSAGE_MAP(CTreeCtrlNodeEx, CTreeCtrl)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
bool CTreeCtrlNodeEx::load_from_file(bool load, CString sfile)
{
...
BOOL CTreeCtrlNodeEx::OnEraseBkgnd(CDC* pDC)
{
...
void CTreeCtrlNodeEx::on_mouse_event(int mouse_event, CPoint point)
{
...
void CTreeCtrlNodeEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CTreeCtrlNodeEx::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CTreeCtrlNodeEx::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CTreeCtrlNodeEx::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
BOOL CTreeCtrlNodeEx::PreTranslateMessage(MSG* pMsg)
{
...
void CTreeCtrlNodeEx::use_check_box(bool use)
{
	m_use_check_box = use;
	Invalidate();
}

void CTreeCtrlNodeEx::use_check_box_at_root(bool use)
{
	m_check_box_at_root = use;
	Invalidate();
}

void CTreeCtrlNodeEx::set_check_box_size(int size /*= 16*/)
{
	if (size < 8)
		size = 16;
...
void CTreeCtrlNodeEx::always_show_selection(bool show)
{
...
void CTreeCtrlNodeEx::reconstruct_font()
{
	m_font.DeleteObject();
...
int CTreeCtrlNodeEx::get_font_size()
{
...
void CTreeCtrlNodeEx::set_font_size( int font_size )
{
	if (font_size == 0)
		return;
...
void CTreeCtrlNodeEx::enlarge_font_size(bool enlarge)
{
	m_font_size = get_font_size();
...
void CTreeCtrlNodeEx::set_font_name(LPCTSTR sFontname, BYTE byCharSet)
{
	if (sFontname == _T(""))
		return;
...
void CTreeCtrlNodeEx::set_font_bold( bool bBold )
{
	m_lf.lfWeight = ( bBold ? FW_BOLD : FW_NORMAL );
	reconstruct_font();
}

void CTreeCtrlNodeEx::set_font_italic(bool italic)
{
	m_lf.lfItalic = italic;
	reconstruct_font();
}

void CTreeCtrlNodeEx::set_log_font(LOGFONT lf)
{
...
void CTreeCtrlNodeEx::set_back_color(COLORREF crBack)
{
	m_crBack = crBack;
	Invalidate();
}

void CTreeCtrlNodeEx::set_text_color(COLORREF crText)
{
	m_crText = crText;
	Invalidate();
}
void CTreeCtrlNodeEx::set_check_color(COLORREF crCheck)
{
	m_crCheck = crCheck;
	Invalidate();
}

void CTreeCtrlNodeEx::PreSubclassWindow()
{
	// TODO: Add your specialized code here and/or call the base class
...
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::get_item_by_index(int index)
{
	int item_count = 0;
	tree<CTreeItem>::pre_order_iterator it = m_tr.begin();
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
...
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::get_next_visible_item(tree<CTreeItem>::pre_order_iterator it)
{
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
...
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::get_previous_visible_item(tree<CTreeItem>::pre_order_iterator it)
{
...
int CTreeCtrlNodeEx::get_visible_item_count()
{
	int item_count = 0;
	tree<CTreeItem>::pre_order_iterator it = m_tr.begin();
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
...
/*
HTREEITEM CTreeCtrlNodeEx::find_item(CString text, HTREEITEM hItem)
{
	if (hItem == NULL)
		hItem = GetRootItem();
...
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::find_item(const CString& findText, int check, int select)
{
	tree<CTreeItem>::pre_order_iterator it = m_tr.begin();
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
...
//findText가 ""이 아니면 해당 아이템만 찾아서 check.
//단, check가 0또는 1이 아니라면 단순히 findText의 노드를 찾고자 하는 목적으로 이용된다.
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::find_item(const CString& parent, const CString& findText, int check, int select)
{
...
tree<CTreeItem>::pre_order_iterator CTreeCtrlNodeEx::get_selected_item(tree<CTreeItem>::pre_order_iterator it)
{
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
...
std::deque<CTreeItem> CTreeCtrlNodeEx::get_checked_item(tree<CTreeItem>::pre_order_iterator it,
...
std::deque<CString> CTreeCtrlNodeEx::get_checked_item_text(tree<CTreeItem>::pre_order_iterator it,
	bool include_current_node, bool only_current_group, bool include_sub_nodes)
{
...
int CTreeCtrlNodeEx::get_checked_count(tree<CTreeItem>::pre_order_iterator it, bool include_current_node, bool only_current_group, bool include_sub_nodes)
{
...
void CTreeCtrlNodeEx::check_item(tree<CTreeItem>::pre_order_iterator it, int check)
{
	it->set_check(check);
	InvalidateRect(get_item_rect(it, false));
}

void CTreeCtrlNodeEx::select_item(tree<CTreeItem>::pre_order_iterator it, bool single_select)
{
	if (single_select)
		find_item(_T(""), -1, 0);
...
CRect CTreeCtrlNodeEx::get_item_rect(tree<CTreeItem>::pre_order_iterator itFind, bool text_only)
{
	CRect	rc;
...
CString CTreeCtrlNodeEx::get_text(tree<CTreeItem>::pre_order_iterator it)
{
	return it->text;
}

void CTreeCtrlNodeEx::ensure_visible(tree<CTreeItem>::pre_order_iterator it)
{
	CRect rc;
	CRect rect;
	int offset = 0;
...
void CTreeCtrlNodeEx::use_expand_button(bool use)
{
...
void CTreeCtrlNodeEx::expand(tree<CTreeItem>::pre_order_iterator it, int expand)
{
	if (it.number_of_children() == 0)
	{
		it->expanded = false;
...
void CTreeCtrlNodeEx::recalculate_scroll_size()
{
	CRect	rc;
	GetClientRect(rc);
...
void CTreeCtrlNodeEx::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: Add your message handler code here and/or call default
	SCROLLINFO	si;
	si.cbSize = sizeof( SCROLLINFO );
...
BOOL CTreeCtrlNodeEx::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	// TODO: Add your message handler code here and/or call default
...
void CTreeCtrlNodeEx::OnWindowPosChanging(WINDOWPOS* lpwndpos)
{
...
void CTreeCtrlNodeEx::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
...
void CTreeCtrlNodeEx::use_scrollbar(bool use)
{
	m_use_scrollbar = use;
...
void CTreeCtrlNodeEx::set_color_theme(int theme, bool invalidate)
{
	switch ( theme )
	{
...
void CTreeCtrlNodeEx::draw_line_node(bool draw)
{
...
void CTreeCtrlNodeEx::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CTreeCtrlNodeEx::OnSetFocus(CWnd* pOldWnd)
{
...
void CTreeCtrlNodeEx::OnKillFocus(CWnd* pNewWnd)
{
...
void CTreeCtrlNodeEx::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
...
void CTreeCtrlNodeEx::recalculate_line_height()
{
...
void CTreeCtrlNodeEx::set_image_list_size(int image_size)
{
...
void CTreeCtrlNodeEx::create_image_list(int image_size)
{
	if (m_image_list)
...
void CTreeCtrlNodeEx::set_image_list_ID(std::deque<UINT>& dq_image_ID)
{
...
void CTreeCtrlNodeEx::set_image_random()
{
	tree<CTreeItem>::pre_order_iterator it = m_tr.begin();
	tree<CTreeItem>::pre_order_iterator end = m_tr.end();
