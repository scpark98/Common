...
//NHÀÇ LiveWebClient¿¡¼­ »ç¿ëÇÏ´Â opencv¿Í ¹º°¡ Ãæµ¹ÀÌ »ý±ä´Ù.
...
//"c:"		"\\abc/def\\"		"123"		".txt" °ú °°ÀÌ ºÐ¸®µÇ´Âµ¥ ±âÁ¸¿¡ »ç¿ëÇÏ´ø ±â´ë°ª°ú ´Þ¶ó º¸Á¤ÇÑ´Ù.
...
//ÆÄÀÏÀÎÁö Æú´õÀÎÁö¸¦ ±¸ºÐÇØ¼­ Ã³¸®ÇÏ´Â ÄÚµå´Â ÇÊ¼ö´Ù.(½ÇÁ¦ Á¸ÀçÇÏ´Â °æ¿ì¿¡¸¸ °Ë»ç°¡ °¡´ÉÇÏ´Ù)
//´Ü, path°¡ "¿¬±¸¼Ò¹®¼­(\\192.168.1.103) (Y:)"°ú °°ÀÌ ³×Æ®¿öÅ© °æ·Î¸¦ Æ÷ÇÔÇÑ µå¶óÀÌºê º¼·ýÀÎ °æ¿ì´Â
//ºÐ¸®ÇØ¼­´Â ¾ÈµÇ¹Ç·Î ±×³É ¸®ÅÏÇØ¾ß ÇÑ´Ù.
CString		get_part(CString path, int part)
{
	...
	//Æú´õ¸íÀº µå¶óÀÌºê °æ·Î±îÁö ¸ðµÎ Æ÷ÇÔ. "\\abc/def\\" => "c:\\abc/def\\"
	...
	//È®ÀåÀÚ´Â .À» Á¦¿Ü½ÃÅ²´Ù. parts[3]°¡ ""ÀÌ¾îµµ .Mid(1)Àº ¿¡·¯°¡ ¹ß»ýÇÏÁö´Â ¾Ê´Â´Ù.
	...
	if (PathFileExists(path))
	{
		...
		{
			//ÇØ´ç °æ·Î°¡ µð·ºÅä¸®ÀÌ°í part == fn_nameÀÌ¶ó°í ³Ñ¾î¿Ô´Ù¸é
...
//È®ÀåÀÚ ÁýÇÕ ¹®ÀÚ¿­·Î ÆÄÀÏ¿­±â ´ëÈ­»óÀÚÀÇ filter stringÀ» ¸®ÅÏÇÑ´Ù.
...
//comma			: Á¤¼ö ºÎºÐ¿¡ ÀÚ¸®¼ö ÄÞ¸¶¸¦ Ç¥½ÃÇÒ Áö
...
//check_prefix°¡ trueÀÌ¸é http, https±îÁö Ã¼Å©ÇÑ´Ù. ¹º°¡ Ãë¾àÁ¡ÀÌ ÀÖ´ÂµíÇÏ¿© ¿ì¼± »ç¿ë±ÝÁö.(https://mathiasbynens.be/demo/url-regex)
...
bool parse_url(CString full_url, CString& ip, int& port, CString& sub_url, bool &is_https)
{
	...
	//AfxParseURL()À» »ç¿ëÇÏ±â À§ÇØ¼­´Â url¿¡ ¹Ýµå½Ã http:// ¶Ç´Â https:// µî°ú °°Àº ¼­ºñ½º Á¾·ù°¡ Ç¥½ÃµÇ¾î¾ß ÇÑ´Ù.
...
//local_file_path°¡ ""ÀÌ¸é °á°ú°ªÀ» ¹®ÀÚ¿­·Î ¸®ÅÏ¹Þ´Â´Ù.
//local_file_path°¡ ÁöÁ¤µÇ¾î ÀÖÀ¸¸é ÆÄÀÏ·Î ´Ù¿î¹Þ´Â´Ù. (ÀÌ¶§ result_strÀº "")
//¸®ÅÏ°ªÀÌ 200ÀÌ ¾Æ´Ò °æ¿ì´Â ¸®ÅÏµÈ ¿¡·¯ÄÚµå¿Í result_str¿¡ ÀúÀåµÈ ¿¡·¯ ¸Þ½ÃÁö¸¦ Á¶ÇÕÇÏ¿© ¿¡·¯ Ã³¸®ÇÑ´Ù.
...
void request_url(CRequestUrlParams* params)
{
	...
	//¶ÇÇÑ ip¿¡ http:// ¶Ç´Â https:// °¡ ºÙ¾î ÀÖÀ¸¸é InternetConnect()Àº ½ÇÆÐÇÑ´Ù. Á¦°ÅÇÏ°í È£ÃâÇØÁà¾ß ÇÑ´Ù.
	...
	//sub_urlÀÇ ¸Ç ¾Õ¿¡´Â ¹Ýµå½Ã '/'°¡ ºÙ¾îÀÖ¾î¾ß ÇÑ´Ù.
	if (params->sub_url[0] != '/')
	...
	//0¹ÙÀÌÆ®ÀÇ ÆÄÀÏÀº ´Ù¿î¹ÞÁö ¾Ê¾Æµµ µÉ µí ÇÏÁö¸¸
	//¼­¹öÀÇ ÆÄÀÏ°ú ´Ù¿î¹ÞÀº ·ÎÄÃÀÇ ÆÄÀÏÀÇ ¼ö°¡ °°ÀºÁö µîÀ» ºñ±³ÇÒ ¼öµµ ÀÖÀ¸¹Ç·Î »ý¼ºÇÏÀÚ.
	//HTTP_QUERY_FLAG_NUMBERÀ» ³ÖÁö ¾ÊÀ¸¸é HttpQueryInfo()¿¡¼­ ¿À·ù°¡ ¹ß»ýÇÑ´Ù.
	...
	do
	{
		...
		//¹öÆÛ Å©±â°¡ 1K¸é ³Ê¹« ºó¹øÇÑ read°¡ ¹ß»ýÇÏ¿© ´À¸®Áö¸¸ 4KÀÌ»óÀÌ¸é Å« Â÷ÀÌ´Â ¹ß»ýÇÏÁö ¾Ê´Â´Ù.
		...
		if (params->local_file_path.IsEmpty())
		{
			//UTF-8 with BOMÀ¸·Î ÀÛ¼ºµÈ txt ÆÄÀÏÀ» ÀÐ¾î¿À¸é ÆÄÀÏ Çì´õ¿¡ EF BB BF ¶ó´Â 3 char°¡ ºÙ¾î¿Â´Ù.
			//ÀÌ´Â À©µµ¿ì¿¡¼­ fopenÀ¸·Î ÀÐ¾î¿Ã¶§´Â ¹®Á¦µÇÁö ¾ÊÀ¸³ª
			//InternetReadFile()·Î ÀÐ¾î¿Í¼­ ÀúÀåÇÏ¸é ¸Ç ¾Õ ±ÛÀÚ°¡ ±úÁ® Ç¥½ÃµÇ´Â ¹®Á¦°¡ ¹ß»ýÇÑ´Ù. ³¯·ÁÁØ´Ù.
	...
	if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == NO_ERROR)
	{
 		...
 		while (pAdapter)
		{
			...
			if (pAdapter->Type == MIB_IF_TYPE_ETHERNET && CString(pInfo->sDescription).Find(sTargetDeviceDescription) >= 0)
			{
				...
				//TCHAR sMacAddress[16] ÀÌ¹Ç·Î ±¸ºÐÀÚ¸¦ ³Ö¾îÁÖ¸é ¾ÈµÈ´Ù.
				//main¿¡¼­ Functions.hÀÇ get_mac_address_format()ÇÔ¼ö·Î ±¸ºÐÀÚ¸¦ ³Ö¾î¼­ Ç¥½ÃÇÒ °Í.
...
//IPv4 ¹®ÀÚ¿­À» ¼ýÀÚ·Î ¹Ù²Ù´Â ¹ü¿ë ÄÚµåÀÌ¹Ç·Î ¹öÀü ¹®ÀÚ¿­ µî ÀÏ¹Ý ¹®ÀÚ¿­ ºñ±³¿¡ »ç¿ëÇÏÁö ¸»°Í.
...
//blur°¡ »ý±â°Ô ÇÏ°í ÀÌ¸¦ shadow·Î »ç¿ëÇÏ´Â ¹æ½ÄÀÎµ¥ ¹º°¡ ¾î»öÇÏ´Ù.
...
int RenameFiles(CString folder, CString oldName, CString newName, bool overwrite, bool bWholename, bool bRecursive)
{
	...
	for (int i = 0; i < files.size(); i++)
	{
		//°Ë»öµÈ ÆÄÀÏ¸í¿¡¼­ oldName ´ÙÀ½¿¡ .°ú _Snapshot ÀÌ µÎ°³ÀÇ ÆÐÅÏÀº °°Àº ÆÄÀÏ±ºÀ¸·Î Ã³¸®ÇÑ´Ù.
...
int get_token_string(CString src, std::deque<CString> &dqToken, CString separator, bool allowEmpty, int nMaxToken, bool include_rest)
{
	...
	while (true)
	{
		...
		if (nMaxToken > 0 && dqToken.size() == nMaxToken)
		{
			...
			//³ª¸ÓÁö ¹®ÀÚ¿­µµ ¸ðµÎ ¸Ç ¸¶Áö¸· ÅäÅ«µÚ¿¡ ºÙ¿©ÁØ´Ù.
...
// lfrf´Â ¶óÀÎºÐ¸®¹®ÀÚ¿­ÀÌ°í º¸Åë "\n"ÀÌ°Å³ª "\r\n" µîÀÌ ÀÖ°í
// separator´Â ':' ÀÌ¸§°ú °ªÀ» ±¸ºÐÇÏ´Â ±¸ºÐÀÚÀÌ´Ù.
...
bool get_bracket_token(CString src, std::deque<CString>* token, TCHAR bracket)
{
	...
	while (true)
	{
		...
		//½ÃÀÛ °ýÈ£´Â ÀÖ´Âµ¥ ³¡ °ýÈ£°¡ ¾ø´Ù¸é Á¤»óÀÌ ¾Æ´Ï´Ù.
...
//ÀÌ °ªÀº À©µµ¿ì°¡ ¼³Ä¡µÉ ¶§ »ý¼ºµÇ°í Àç¼³Ä¡µÇÁö ¾ÊÀ¸¸é À¯ÁöµÈ´Ù.
...
bool get_windows_update_setting(bool& auto_update, int& level)
{
	...
	if (SUCCEEDED(CoInitializeEx(NULL, COINIT_APARTMENTTHREADED)))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				//AfxMessageBox(_T("ÀÚµ¿ ¾÷µ¥ÀÌÆ® »óÅÂ¸¦ °¡Á®¿À´Â µ¥ ½ÇÆÐÇß½À´Ï´Ù."));
				return false;
			}
...
//¼³Á¤½Ã°£°ú Àá±ÝÈ­¸éÀ» Ç¥½ÃÇÒÁö¿¡ ´ëÇÑ ¼³Á¤°ªÀ» ¾ò¾î¿Ã ¼ö ÀÖ´Ù.
...
//¿Ö ÀÌ·± Â÷ÀÌ°¡ ¹ß»ýÇÏ´ÂÁö´Â ¾ÆÁ÷ ¾Ë ¼ö ¾øÀ¸³ª
...
CString	get_drive_volume(TCHAR drive_letter)
{
	...
	*/
	...
	if (sLabel.IsEmpty())
	{
		sLabel.Format(_T("%s (%c:)"), GetUserDefaultUILanguage() == 1042 ? _T("·ÎÄÃ µð½ºÅ©") : _T("Local Disk"), drive_letter);
	}
	else
	{
...
//"·ÎÄÃ µð½ºÅ© (C:)" <-> "C:\\" //ÇÏÀ§ Æú´õ Æ÷ÇÔ À¯¹«¿¡ °ü°è¾øÀÌ º¯È¯
...
//char chStr[100] = { 0, };¿Í °°ÀÌ pointer º¯¼ö°¡ ¾Æ´Ñ ¹è¿­·Î ¼±¾ðµÈ °æ¿ì¶ó¸é
//chStr = CString2char(str); ¹®ÀåÀº ¿À·ù°¡ ¹ß»ýÇÏ¹Ç·Î ¾Æ·¡¿Í °°ÀÌ »ç¿ëÇÒ °Í.
//sprintf(chStr, "%s", (LPSTR)(LPCTSTR)str);	//MBCS : ok, UNICODE : fail
//sprintf(chStr, "%s", CStringA(str));		//both : ok
//¸®ÅÏ¹Þ¾Æ »ç¿ëÇÑ char* º¯¼ö°ªÀº »ç¿ë ÈÄ ¹Ýµå½Ã delete [] ÇØÁÙ°Í
...
#if defined(UNICODE) || defined(_UNICODE)
	//length°¡ -1ÀÎ °æ¿ì, Áï ±æÀÌ¸¦ ¸ð¸£°Å³ª Æ¯º°È÷ ÁöÁ¤ÇÏÁö ¾Ê°í
...
//ÇöÀç °¡¿ë ¸Þ¸ð¸®¸¦ ¸®ÅÏÇÑ´Ù. (total_memory : ÀüÃ¼ ¸Þ¸ð¸® ¿ë·®)
...
CString	GetDayTimeCountString(int format, CTimeSpan ts, bool bShowZero, bool bIncludeSec)	//ts°ªÀ» ³Ñ°Ü ¹Þ¾Æ "aÀÏ b½Ã°£ cºÐ dÃÊ" ÇüÅÂ·Î Ç¥½Ã
{
	CString str;
	CString sResult = _T("");
...
//src³»ÀÇ ¸ðµç ¹®ÀÚ¿¡ ´ëÇØ digitsÀÚ¸´¼öÀÇ Á¶ÇÕ »ý¼º
...
CString run_process(CString exePath, bool wait_process_exit, bool return_after_first_read)
{
	...
	//dir°ú °°Àº DOS ¸í·ÉÀº ¿ÜºÎ ½ÇÇàÆÄÀÏÀÌ ¾Æ´Ï¹Ç·Î exePath¸¦ "dir c:\\*.*"°ú °°ÀÌ ÁÖ¸é ¿¡·¯°¡ ¹ß»ýÇÑ´Ù.
	//"cmd.exe /c dir c:\\*.*"¿Í °°ÀÌ Àü´ÞÇØ¾ß ÇÑ´Ù.
	...
	if (!CreateProcess(NULL,
		(TCHAR*)(const TCHAR*)(exePath),//_T("dir")°ú °°ÀÌ »ó¼ö°ªÀ» Á÷Á¢ ÁÖ¸é ½ÇÆÐ.	// Command line
...
//µû¶ó¼­ topÀÇ °æ¿ì´Â 4ÇÈ¼¿·Î °íÁ¤ÇÏ´Â °ÍÀÌ ÀûÀýÇÏ´Ù.
...
HBITMAP capture_screen_to_bitmap(LPRECT pRect, UINT id, int dx, int dy)
{
	...
	if (pRect)
	{
		...
		//scpark ¸ð´ÏÅÍ À§Ä¡ ¼³Á¤¿¡ µû¶ó À½¼öÀÏ ¼ö ÀÖ´Ù. ¾Æ·¡ º¸Á¤Àº ÇÏÁö ¾Ê¾Æ¾ß ÇÑ´Ù.
		//if (nX < 0) nX = 0;
		//if (nY < 0) nY = 0;
	}
	else
...
HBITMAP	PrintWindowToBitmap(HWND hTargetWnd, LPRECT pRect)
{
	...
	{
		//ÀüÃ¼È­¸éÀÏ¶§¿Í Ã¢¸ðµåÀÏ¶§ ºÒÇÊ¿äÇÑ ¿©¹éÀ» Àß¶ó³»´Â ÄÚµåµéÀÌ³ª
		//¿©¹éÀ» Àß¶ó³»¸é ÇØ´ç Å©±â·Î mpeg³ìÈ­µÇÁö ¾Ê´Â ¿¡·¯°¡ ¹ß»ýÇÏ¿©
...
//return¹ÞÀº char*´Â ¹Ýµå½Ã »ç¿ë ÈÄ free()ÇØÁà¾ß ÇÔ.
...
//´Ü, Win32APIÀÎ EnumDisplayMonitors()¸¦ È£ÃâÇÒ¶§´Â ¹Ýµå½Ã g_dqMonitors.clear()¸¦ ÇØÁà¾ß ÇÏ¹Ç·Î
...
//rÀÌ °ÉÃÄÀÖ´Â ¸ð´ÏÅÍ ÀÎµ¦½º¸¦ ¸®ÅÏ. °ãÃÄÁö´Â ¿µ¿ªÀÌ ¾îµð¿¡µµ ¾ø´Ù¸é -1À» ¸®ÅÏ.
...
{
	//ÃÖ»óÀ§·Î ¿Ã¸®´Â ÄÚµå°¡ ÀÌ ÇÔ¼öÀÇ ¸Ç Ã³À½ ºÎºÐ¿¡¼­ ½ÇÇàµÇÁö ¾ÊÀ¸¸é
...
//.À» ¾ø¾Ø ¼ýÀÚ·Î ºñ±³ÇßÀ¸³ª ÀÌ ¹æ¹ýµµ ¿À·ù ¹ß»ý(1.0.1.13 > 1.0.10.3º¸´Ù Å©´Ù°í ÆÇ´ÜÇÔ)
//°á±¹ °¢ ÀÚ¸´¼ö³¢¸® ±¸ºÐÇÏ°Å³ª ÀÚ¸´¼ö¸¦ ¸ÂÃá ÈÄ ºñ±³ÇÑ´Ù.
 ...
 * @brief HTTP POST ¸Þ¼Òµå·Î ÆÄÀÏÀ» ¾÷·ÎµåÇÑ´Ù.
	...
	else
	{
		...
		if (pclsHttpConn)
		{
			...
			if (pclsHttpFile)
			{
				...
				{
					// À¥¼­¹ö ¿¬°á¿¡ ½ÇÆÐÇÏ¸é CInternetException ÀÌ ¹ß»ýÇÑ´Ù.
				}
...
bool HttpDownloadFile(CString url, CString local_path)
{
	...
	if (m_pHttpFile)
	{
		if (!FileWrite.Open("c:\\scpark\\¸ðµâ.pdf", CFile::modeCreate | CFile::modeReadWrite | CFile::shareDenyNone))
		{
			//exception found
			//lots of clean up code
			return false;
...
{
	//VS¿¡¼­ ÀÌ ÇÁ·ÎÁ§Æ®¸¦ ½ÇÇàÇÏ¿© WebView2¸¦ ¼³Ä¡ÇÏ¸é HKEY_CURRENT_USER¿¡ ±â·ÏÀÌ ¹Ý¿µµÇ°í
	//urlschemeÀ» ÅëÇØ¼­ KoinoAVCSupporter.exe -> WebView2¸¦ ¼³Ä¡ÇÏ¸é HKEY_LOCAL_MACHINE¿¡ ±â·ÏÀÌ ³²´Â´Ù.
	//webview2¸¦ uninstallÇÏ¸é(Microsoft Edge WebView2 ·±Å¸ÀÓ) µÎ °÷ ¸ðµÎ Á¦°ÅµÈ´Ù.
...
bool install_WebView2Runtime(CString runtimeExePath, bool silentInstall)
{
	...
	HWND hWnd = NULL;
	int wait_count = 0;
	//¼³Ä¡°¡ ¿Ï·áµÇ¸é ¼³Ä¡¿Í °ü·ÃµÈ Ã¢Àº ¸ðµÎ ´Ý¾ÆÁØ´Ù.
	do
	{
...
//ÄÜ¼Ö ¸í·ÉÀÎÁö À©µµ¿ì ÇÁ·Î±×·¥ÀÎÁö ±¸ºÐ
