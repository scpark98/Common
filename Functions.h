...
//폴더 관련
	...
	//20260704 by claude. 드래그 중 hover 대상 컨트롤을 벗어났을 때, 그 컨트롤의 drop-highlight 를 컨트롤 타입에 맞게 해제한다.
	//CListCtrl(파생 포함)=전 항목 LVIS_DROPHILITED 해제 + Invalidate, CTreeCtrl(파생 포함)=SelectDropTarget(NULL).
	//pWnd 가 NULL 이거나 둘 다 아니면 무시. cross-control 드래그에서 이전 하이라이트가 잔존하는 것을 막는 용도.
...
*/
	...
	void		draw_line(ID2D1DeviceContext* d2dc, float x1, float y1, float x2, float y2, Gdiplus::Color cr, float thick = 1.0f);
