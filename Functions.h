...
public:
	...
	CString get_exif_str()
	{
		...
		{
			gps_info.Format(_T("\nGPS Á¤º¸: %s, %s, %.0fm"), gps_latitude_str, gps_longitude_str, gps_altitude);
			res += gps_info;
		}
...
//request_url() ³»ºÎ¿¡¼­ ¶Ç´Â ´Üµ¶ÀûÀ¸·Î ¼­¹ö°¡ ÇöÀç ±âµ¿ÁßÀÎÁö¸¦ ºü¸£°Ô ÆÇº°ÇÏ±â À§ÇÑ ÇÔ¼ö
bool		is_server_reachable(CString ip, int port, int timeout_ms = 1000);

// ¼­¹ö µµ´Ş °¡´É ¿©ºÎ¸¦ °øÀ¯ Ä³½Ã·Î °ü¸®ÇÏ´Â Å¬·¡½º
// 529Çà ±×´ë·Î
class CRequestUrlParams;
...
public:
	...
	void update(bool success);		// ¡Ú ¼±¾ğ¸¸ (±¸ÇöÀº CRequestUrlParams Á¤ÀÇ ÀÌÈÄ)
...
public:
	...
	// Ãë¼Ò Áö¿ø: request_url ³»ºÎ¿¡¼­ InternetOpen ÇÚµéÀ» ÀúÀåÇÏ°í,
	// ¿ÜºÎ¿¡¼­ cancel()À» È£ÃâÇÏ¸é ÇÚµéÀ» ´İ¾Æ ºí·ÎÅ· WinInet ÀÛ¾÷À» Áï½Ã Ãë¼ÒÇÑ´Ù.
	std::atomic<HINTERNET> h_cancel_root{ nullptr };
	void cancel()
	{
		HINTERNET h = h_cancel_root.exchange(nullptr);
		if (h) InternetCloseHandle(h);
		// MSDN: ºÎ¸ğ ÇÚµéÀ» ´İÀ¸¸é ÀÚ½Ä ÇÚµé(hConnect, hRequest)µµ ¹«È¿È­µÊ
	}
	...
	CString		verb = _T("GET");
	//urlÀÇ ½ÃÀÛÀÌ httpÀÎÁö httpsÀÎÁö, port°¡ 80 ¶Ç´Â 443ÀÏ °æ¿ì´Â ÀÚµ¿ ¼³Á¤µÇÁö¸¸
	//ÀÓÀÇ Æ÷Æ®¹øÈ£¸¦ »ç¿ëÇÏ´Â °æ¿ì¿¡´Â ¹İµå½Ã ¸í½ÃÇØÁà¾ß ÇÑ´Ù.
	bool		is_https = true;
	CString		body;					//post data(json format)

	//default = 30ÃÊ
	//¿¬°á Å¸ÀÓ¾Æ¿ô°ú Àü¼Û Å¸ÀÓ¾Æ¿ô ºĞ¸®.
...
// ¡Ú CRequestUrlParams ¿ÏÀü Á¤ÀÇ ÀÌÈÄ¿¡ ±¸Çö
...
//ÇÁ·Î¼¼½º °ü·Ã
	...
	//Æ¯Á¤ À©µµ¿ì°¡ ¼ÓÇØÀÖ´Â ¸ğ´ÏÅÍ ÀÎµ¦½º¸¦ ¸®ÅÏ
...
//¹®ÀÚ¿­
	...
	//À½¼öÀÎ °æ¿ìµµ ÀÖÀ¸¹Ç·Î unsigned·Î ÇÏ¸é ¾ÈµÊ.
	...
	//ÀÌ ÇÔ¼ö´Â ÀÚ¸´¼ö´Â Æ²·Áµµ ¹«°üÇÏ³ª separator°¡ ¼ıÀÚ¶óµçÁö, ¼ıÀÚ°¡ ¾Æ´Ñ ¹®ÀÚ°¡ ¼¯¿©ÀÖÀ» °æ¿ì¿¡ ´ëÇÑ Ã³¸®´Â ¾ø´Ù.
	//¹öÀüÀÌµç IPÁÖ¼ÒÀÌµç Á¤¼®ÀûÀÎ ¹®ÀÚ¿­ÀÌ µé¾î¿Ô´Ù¶ó´Â °¡Á¤ÇÏ¿¡ Á¤È®È÷ ÆÇ´ÜÇÑ´Ù.
...
//Æú´õ °ü·Ã
	...
	enum FILENAME_PART
	{
		...
		fn_folder,			//fullpath Æú´õ¸íÀÌ¹Ç·Î µå¶óÀÌºê °æ·Î±îÁö ¸ğµÎ Æ÷ÇÔµÈ´Ù.	
	...
	//°¢ ¶óÀÎÀÇ ³¡¿¡ ÀÖ´Â '\n' ±âÈ£´Â ±¸ºĞÀÚÀÌ¹Ç·Î deque¿¡ µé¾î°¥ ¶§´Â Æ÷ÇÔµÇÁö ¾ÊÀ½¿¡ ÁÖÀÇ.
...
//ÀÎÅÍ³İ ÆÄÀÏ
	...
	//±âº» 30ÃÊ Å¸ÀÓ¾Æ¿ôÀ¸·Î ¿äÃ»ÇÏ´Âµ¥ °£È¤ ¼­¹ö°¡ ±¸µ¿ÁßÀÎÁö ºü¸£°Ô È®ÀÎÇÏ°íÀÚ ÇÑ´Ù¸é check_server_reachableÀ» true·Î ÇÏ¸é 3ÃÊÀÌ³» ÆÇº°µÈ´Ù.
	//´Ü, ÀÌ ¿äÃ»Àº º°µµÀÇ TCP ¼ÒÄÏÀ» »ı¼ºÇÏ¹Ç·Î ´Ù¼öÀÇ ¿äÃ»ÀÌ ¹ß»ıÇÏ´Â »óÈ²¿¡¼­´Â Àı´ë »ç¿ëÇÏ¸é ¾ÈµÈ´Ù.
...
//½©(shell), À©µµ¿ì(window), ·¹Áö½ºÆ®¸®(registry), ½Ã½ºÅÛ(system)
	...
	//Á» ´õ Å×½ºÆ® ÇÊ¿ä!
	//½ÇÇàÆÄÀÏ¸íÀ¸·Î À©µµ¿ì ÇÚµé ¸®ÅÏ. ½ÇÇàÆÄÀÏ¸í ¶Ç´Â fullpath·Î °Ë»ö.
...
*/
	...
	//"1d 2h 3m" Çü½ÄÀÇ ¹®ÀÚ¿­À» ºĞÀ¸·Î È¯»ê. ´ë¼Ò¹®ÀÚ ¹«°ü, °ø¹é ¹«½Ã.
	//µ¿ÀÏ ´ÜÀ§ ¹İº¹ µîÀå ½Ã ´©Àû (ex: "1d 2m 3h 4m" = 1d + 3h + 6m = 1626ºĞ).
	//´ÜÀ§ ¾øÀÌ ÀÔ·ÂµÈ ¼ıÀÚ ÅäÅ«Àº ¹«½ÃµÇ¹Ç·Î È£Ãâ Ãø¿¡¼­ »çÀü °Ë»ç ±ÇÀå.
	...
	void		draw_line(Gdiplus::Graphics& g, int x1, int y1, int x2, int y2, Gdiplus::Color cr, float thick = 1.0f, Gdiplus::DashStyle pen_style = Gdiplus::DashStyleSolid, int nDrawMode = R2_COPYPEN, Gdiplus::LineCap line_cap = Gdiplus::LineCapFlat);
...
//Á¾·áµÉ ¶§ ÇØÁ¦½ÃÄÑÁÖ´Â ÇÔ¼öµµ ¹İµå½Ã È£ÃâÇØÁà¾ß ÇÑ´Ù.
	...
	//GetWindowRect()´Â ½ÇÁ¦ º¸¿©Áö´Â À©µµ¿ìÀÇ ¸®¾ó ÁÂÇ¥°¡ ¾Æ´Ñ DWM/Aero ±â¹İÀÇ resize border±îÁö Æ÷ÇÔÇÏ¹Ç·Î º¸Åë 8¾¿ È®ÀåµÈ ¿µ¿ªÀ» ¸®ÅÏÇÑ´Ù.
	//Æ¯È÷ ÀüÃ¼È­¸éÀÏ °æ¿ì lt´Â 0,0ÀÌ ¾Æ´Ñ -8,-8·Î ¸®ÅÏµÈ´Ù. ½ÇÁ¦ º¸¿©Áö´Â ¿µ¿ªÀ» ±¸ÇÏ±â À§ÇÑ ÇÔ¼ö
