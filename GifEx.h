//////////////////////////////////////////////////////////////////////
// PictureEx.h: interface for the CGifEx class.
//
...
/*
[scpark 2020 05 11 21]
CPictureEx는 CStatic을 상속받아
loading, PrepareDC, animate_draw_thread 모두 수행하는 클래스인데
이를 수정하여 loading과 PrepareDC는 그대로 이용하고
이 클래스를 사용하는 메인에서 직접 그려주기 위해 수정했다.
*/
...
public:
	...
	BOOL Load(LPCTSTR szFileName, HWND hWnd, bool onlyOne = false);
	...
	BOOL Load(HGLOBAL hGlobal, DWORD dwSize, bool onlyOne = false);

	// loads a picture from a program resource
	// i.e. Load(MAKEINTRESOURCE(IDR_MYPIC),_T("GIFTYPE"));
	BOOL Load(LPCTSTR szResourceName,LPCTSTR szResourceType);

	HDC			get_hMemDC() { return m_hMemDC; }
	HBITMAP		get_hBitmap() { return m_hBitmap; }
	IPicture* get_picture() { return m_pPicture; }
	std::deque<CGifEx::TFrame>* get_frames_deque() { return &m_arrFrames; }
protected:
	...
	void redraw_image();
	BOOL PrepareDC(int nWidth, int nHeight, bool onlyOne = false);
