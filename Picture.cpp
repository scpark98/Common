...
BOOL CPicture::Draw(CDC* pDC, int x, int y, int nWidth, int nHeight)
{
	CRect r(x, y, x + nWidth, y + nHeight );
...
// ~~~~~~
//-----------------------------------------------------------------------------
BOOL CPicture::Draw(CDC *pDC, CRect DrawRect, bool bAspectRatio /*= true*/ )
//=============================================================================
{
    if (pDC == NULL || m_IPicture == NULL) return FALSE;
    
    long Width  = 0;	//이미지 가로 크기
    long Height = 0;	//이미지 세로 크기
	...
	if ( bAspectRatio )
	{
		...
		else if ( dRatio < 1.0 )
		{
			...
			newRect.left = DrawRect.left + ( DrawRect.Width() - new_Width ) / 2.0;
			newRect.right = newRect.left + new_Width;
		}
	}
    ...
    hrP = m_IPicture->Render(pDC->m_hDC,
                      newRect.left,                  // Left
                      newRect.top,                   // Top
                      newRect.right - newRect.left, // Right
                      newRect.bottom - newRect.top, // Bottom
                      0,
                      Height,
                      Width,
                      -Height,
...
//=============================================================================
{
   ...
   if(BMP.LoadBitmap(BMPResource))
		...
		{
		TRACE(_T("ERROR: Can Not Find The Bitmap Resource\n"));
		return(FALSE);
		}
