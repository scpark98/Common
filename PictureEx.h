...
public:
	...
	//Load시에 표시 크기 기능을 추가했으며
	//현재로는 로딩시에 그 표시 크기를 줘야한다.
	//로딩후에 크기 조정 기능은 미구현.
	// 
	// loads a picture from a file
	// i.e. Load(_T("mypic.gif"));
	BOOL Load(LPCTSTR szFileName, CSize displaySize = CSize(0, 0));
	...
	BOOL Load(HGLOBAL hGlobal, DWORD dwSize, CSize displaySize = CSize(0, 0));

	// loads a picture from a program resource
	// i.e. Load(MAKEINTRESOURCE(IDR_MYPIC),_T("GIFTYPE"));
	//확장자가 .gif인 파일을 리소스 뷰에 등록하면 VS가 BMP로 변경시키는 버그가 있다.
	//반드시 .gif 확장자를 .bin과 같이 다른 확장자로 변경한 후 등록해서 사용해야 한다.
	BOOL Load(LPCTSTR szResourceName,LPCTSTR szResourceType, CSize displaySize = CSize(0, 0));

	int width() { return GetSize().cx; }
	int height() { return GetSize().cy; }
...
protected:
	...
	HGLOBAL GetNextGraphicBlock(UINT *pBlockLen, UINT *pDelay, 
		SIZE *pBlockSize, SIZE *pBlockOffset, UINT *pDisposal, CSize displaySize = CSize(0, 0));
