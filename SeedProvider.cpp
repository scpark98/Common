...
//파일을 암호화, 복호화 함.
int	SeedProvider::EncryptFileWithHeader(CString src_file, bool overwrite, CString dst_file)
{
	if (PathFileExists(src_file) == false)
	{
	...
	CFile inFile(src_file, CFile::modeRead | CFile::typeBinary);
	...
	CFile outFile(dst_file, CFile::modeCreate | CFile::modeWrite | CFile::typeBinary);
...
int SeedProvider::DecryptFileWithHeader(CString src_file, bool overwrite, CString dst_file)
{
	if (PathFileExists(src_file) == false)
	{
		m_nLastError = SP_ERROR_FILE_NOT_FOUND;
