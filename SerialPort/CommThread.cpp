...
void CCommThread::SetParentHwnd( HWND hWnd )
{
	ASSERT( hWnd != NULL );
...
void CCommThread::SetInstanceID( int nID )
{
	m_nCommID = nID;
}
						   ...
						   DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity )
{
	...
	// Local 변수.
		...
		//--> 포트명 저장. 여기서 역슬래시를 한번 더 써준 이유는 COM10 이상을 쓸때에도 문제가 없기 때문.
		m_sPortName.Format( "\\\\.\\%s", strPortName );
...
DWORD	ThreadWatchComm(CCommThread* pComm)
{
   ...
   while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
   {
		...
		//SetCommMask (pComm->m_hComm, EV_RXCHAR | EV_CTS | EV_DSR | EV_RING);
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			...
			do
			{
				dwRead = pComm->ReadComm( btBuff, BUFF_SIZE ); //들어온 데이터 읽어 오기 

				if ( BUFF_SIZE - pComm->m_QueueRead.GetSize() > (int)dwRead )
				{
					for( WORD i = 0; i < dwRead; i++ )
					{
						pComm->m_QueueRead.PutByte( btBuff[i] );//큐 버퍼에  들어온 데이터 넣기 
					}
				}
				else
				{
					//AfxMessageBox("buffer full"); //큐버퍼의 크기를 초과하면 경고 메시지 보냄
				}
				//Sleep(0);
			}while(dwRead);


			//printf( "dwRead = %d\n", pComm->m_QueueRead.GetSize() );
