...
int inet_pton_1(int af, const char *src, void *dst);
#endif
...
CSocketHandle::~CSocketHandle()
{
	for ( int i = m_arClient.GetCount() - 1; i >= 0; i-- )
		RemoveClient( i );
...
bool CSocketHandle::GetLocalAddress(LPTSTR pszAddress, UINT nSize, int nFamily /*= AF_INET*/)
{
    if (pszAddress != NULL && nSize > 0)
    {
        ...
        if (GetLocalName(szHost, HOSTNAME_SIZE))
        {
            ...
            inet_pton_1(nFamily, pstrHost, &addr_store);
            const void* addr;
...
char* CSocketHandle::GetClientAlias( const SockAddrIn& addrIn )
{
	int nPort = static_cast<int>(static_cast<UINT>(ntohs(addrIn.GetPort())));
	return GetClientAlias( nPort );
}

char* CSocketHandle::GetClientAlias( int nClientPort )
{
	for ( int i = 0; i < m_arClient.GetCount(); i++ )
	{
		CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt(i);
		if ( pClient->m_nPort == nClientPort )
		{
			return pClient->m_sAlias;
		}
	}
...
char* CSocketHandle::GetClientAlias( SOCKET sock )
{
	for ( int i = 0; i < m_arClient.GetCount(); i++ )
	{
		CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt(i);
		if ( pClient->m_Sock == sock )
		{
			return pClient->m_sAlias;
		}
	}
...
SOCKET CSocketHandle::GetSocketByAlias( char* sAlias )
{
	for ( int i = 0; i < m_arClient.GetCount(); i++ )
	{
		CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt(i);

		if ( strcmp( pClient->m_sAlias, sAlias ) == 0 )
		{
			return pClient->m_Sock;
		}
	}
...
char* CSocketHandle::GetClientIPByAlias( char* sAlias )
{
	for ( int i = 0; i < m_arClient.GetCount(); i++ )
	{
		CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt(i);
...
void CSocketHandle::RemoveClient( int nIndex )
{
	if ( nIndex < 0 || nIndex >= m_arClient.GetSize() )
		return;

	//할당받은 클라이언트 구조체 메모리를 해제하고
	CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt( nIndex );
	delete pClient;

	//array에서도 제거한다.
	m_arClient.RemoveAt( nIndex );
}

void CSocketHandle::RemoveClient( CString sAlias )
{
	for ( int i = m_arClient.GetCount() - 1; i >= 0; i-- )
	{
		CClientInfo*	pClient = (CClientInfo*)m_arClient.GetAt( i );
		...
		{
			RemoveClient( i );
			//m_Edit_Log.Append( RGB_WARNING, "%s client is removed.", sAlias );
		}
	}
}
