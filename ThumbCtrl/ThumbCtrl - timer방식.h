...
public:
	...
	int			height;
	int			channel;
	CRect		rect;		//thumb가 실제 그려진 좌표값을 저장해둔다.
...
public:
	...
	enum CONTEXT_MENU
	{
		idToggleIndex = 1,	//간혹 0으로 설정하면 메뉴가 실행되지 않는 현상이 있다. 메뉴 id는 1부터 시작해도 무관하니 항상 1부터 시작하자.
		idPromptMaxThumb,
		idDeleteThumb,
		idRemoveAll,
	};
	...
	void			set_limit_count( int limit_count ) { m_limit_count = limit_count; }	//정해진 개수를 넘어가면 맨 뒤부터(?) 지워준다.
...
//컬러
	...
	int				m_limit_count;	//아이템이 계속 증가하면 메모리도 계속 증가된다. 제한을 둬야 할 경우도 있다. -1이면 제한없음
	bool			m_has_focus;
	...
	int				m_min_width;	//썸네일 1개가 표시될 최소 width(margin*2 + tile.cx)

	//스크롤 기능 관련
	bool			m_bUseMultiline;
	int				m_per_line;		//한 라인에 표시되는 썸네일 개수
	int				m_max_line;		//총 표시해야 될 라인 수
	std::deque<int> m_line_height;	//각 라인마다 타이틀의 길이에 따라 라인의 높이가 다르게 구현할 예정이다.
