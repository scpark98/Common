...
public:
	...
	enum CONTEXT_MENU
	{
		idToggleIndex = 0,	//간혹 0으로 설정하면 메뉴가 실행되지 않는 현상이 있다. 메뉴 id는 1부터 시작해도 무관하니 항상 1부터 시작하자.
		idPromptMaxThumb,
		idDeleteThumb,
		idRemoveAll,
	};
	...
	void			set_limit_count( int limit_count ) { m_limit_count = limit_count; }
...
//타이틀 편집 관련
	...
	void			sort_by_title();
	int				find_by_title(CString title);
...
//다중 파일 로딩 관련
	...
	int				m_nThreadLoadingCompleted;				//완료된 데이터 수
	void			CheckAllThreadEnding();
