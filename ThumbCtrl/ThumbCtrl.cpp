...
void CThumbCtrl::loading_function(int idx)
{
	...
	for (ii = m_nStartIndex[idx]; ii < m_nEndIndex[idx]; ii++)
	{
		...
		{
			mat = loadMat(m_loading_files[ii]);
		}

		insert(ii, mat, m_loading_files[ii], GetFileTitle(m_loading_files[ii]), false, false);
...
int CThumbCtrl::insert(int index, cv::Mat mat, CString full_path, CString title, bool key_thumb, bool invalidate)
{
	...
	//원본을 썸네일의 타일 크기로 resize해서 추가했었으나
	//크기 조정을 할 경우 깨져보이는 부작용이 있다.
	//어짜피 draw_function에서 dc에 그릴 때 StretchDIBits를 사용하므로
	//원본을 유지하도록 수정.
	/*
	int nw, nh;
	CRect r;
...
cv::Mat	CThumbCtrl::get_mat(int index)
{
	if (index < 0 || index >= m_dqThumb.size())
		return cv::Mat();

	return m_dqThumb[index].mat;
}
