...
4. **Common commit/push 는 "기능 검증 → 사용자 푸시 승인" 후에만 (강제)** — 어떤 프로젝트 작업 중 Common 을 수정했더라도, *코드 수정·빌드 통과* 만으로 절대 commit/push 하지 말 것. 반드시 ① **사용자가 실제 동작을 확인** (GUI/시청/저장 등 §5B 의 실검증) 하고 ② **사용자가 명시적으로 "push 하라" 고 지시** 한 뒤에만 commit + push 한다. 빌드 통과 = *컴파일 검증* 일 뿐 기능 검증이 아니다. "작업 끝났으니 푸시" 같은 절차적 자동 푸시 금지 — push 는 외부·되돌리기 어려운 동작이라 사용자 승인이 §5B / 외부동작 규칙보다 절대 우선이다. (cross-project sync·머신 간 sync 의 이점은 *승인된 push* 안에서만 누린다 — sync 를 이유로 미검증·미승인 push 를 정당화하지 말 것.)
