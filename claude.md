...
## 0. 세션 시작 시 이 파일이 로드돼 있어야 한다 (강제, 최우선)

집이든 회사든, **세션 시작 시 `Common/claude.md` 를 로딩하도록 규칙이 적용되어 있지 않다면 그 규칙부터 적용하라.** 다른 어떤 작업보다 먼저 한다.

로딩 경로는 두 겹이며 **둘 다** 있어야 한다.

1. **전역** — `~/.claude/CLAUDE.md` 맨 위의 `@D:/1.Projects_C++/Common/claude.md` 한 줄.
   프로젝트에 `claude.md` 가 없어도 로드되므로 이것이 주 경로다.
   이 전역 파일은 UTF-8 **BOM 없음** 이다. PowerShell 기본 인코딩으로 읽고 쓰면 한글이 깨진다 —
   `[System.IO.File]::ReadAllText/WriteAllText` 에 `UTF8Encoding($false)` 를 명시할 것.
   (집·회사 경로가 동일하므로 같은 줄이 양쪽에서 그대로 동작한다. Google Drive 로 동기화되는
   실체 파일이면 한쪽에서 넣으면 반대쪽에도 따라간다. 동기화가 안 돼 있으면 그 머신에서 직접 넣는다.)
2. **프로젝트별** — `<프로젝트 루트>/claude.md` 의 `@../Common/claude.md` 한 줄.
   프로젝트 고유 사항을 적는 파일이기도 하다. 없으면 만든다.

**점검**: 세션 초반에 이 §0 이 컨텍스트에 있는지 스스로 확인한다. 없다면 위 1·2 를 복구하고,
무엇이 누락돼 있었는지 사용자에게 보고한 뒤 작업을 시작한다.

**Why:** 2026-08-28 SCColorTable — 그 프로젝트에 `claude.md` 가 없고 전역 파일에도 import 가 없어
이 문서 전체가 세션에 로드되지 않았다. §5B-0 *"빌드는 사용자가 한다"* 를 못 읽고 MSBuild 를 두 번
돌린 뒤 사용자가 테스트 중이던 exe 를 강제 종료했다. 사용자 지적 — *"빌드는 내가 한다고
Common/claude.md에 써놓으면 뭐하냐? 어짜피 클로드 너는 안보는데 무슨 의미가 있냐?"*,
*"어떤 프로젝트라도 반드시 Common/claude.md를 읽고 시작하도록 조치하라. 이건 집이든, 회사든
마찬가지다."* 당시 `.sln` 은 있으나 `claude.md` 가 없는 프로젝트가 43개였다.
그때까지 각 프로젝트 `claude.md` 는 "홈 폴더 `~/.claude/claude.md` symlink 로 자동 로드된다" 고
적고 있었으나, 회사 머신 홈에는 `CLAUDE.md`(Google Drive symlink) 만 있고 그 symlink 는 없었다.
무사했던 프로젝트들은 symlink 덕이 아니라 각자 `@../Common/claude.md` 를 갖고 있었기 때문이다.

## 0-1. 모든 프로젝트에 `claude.md` 를 만든다 (기본)

새 프로젝트를 처음 다루기 시작할 때, 루트에 `claude.md` 가 없으면 **만든다.** 첫 줄은 항상
`@../Common/claude.md` (하위 경로면 `@../../Common/claude.md`). 그 프로젝트에 대한 정보·규칙을
담는 파일이며, git 으로 집·회사에 동기화된다.

**담을 것** — 코드를 읽어도 알 수 없는 것만:
- 프로젝트 개요 한두 줄, 절대 경로, **활성 빌드 구성** (`Debug|x64` 등)
- 주요 Common 의존 모듈과 그 역할
- 비자명한 설계 결정과 그렇게 된 경위, 과거 사고 이력, 그 프로젝트에서만 통하는 예외 규칙

**담지 말 것**: 디렉터리 구조·클래스 목록 등 코드에서 바로 읽히는 것, Common 규칙의 재기술.

**크기**: 200줄 이하로 유지한다 (공식 권장 — 길수록 컨텍스트를 먹고 준수율이 떨어진다).
넘어가면 `.claude/rules/*.md` 로 쪼갠다. `paths:` frontmatter 를 주면 해당 파일을 열 때만 로드된다.

### 파일명 — 무엇을 만들어야 자동으로 읽히나

Claude Code 가 세션 시작 시 **자동으로 읽는 파일명은 `CLAUDE.md` 와 `CLAUDE.local.md` 뿐이다.**
위치는 `./CLAUDE.md` 또는 `./.claude/CLAUDE.md`, 그리고 그 위 모든 상위 디렉터리 + `~/.claude/CLAUDE.md`.
하위 디렉터리의 것은 그 폴더 파일을 읽을 때 지연 로드된다.

- **`AGENTS.md` 는 Claude Code 가 읽지 않는다.** 다른 에이전트 도구들의 관례 파일일 뿐이다.
- **Windows 는 대소문자를 구분하지 않으므로 `claude.md` 로 만들어도 `CLAUDE.md` 로 인식된다.**
  집·회사 모두 Windows 이므로 §1 의 소문자 규칙을 그대로 유지해도 안전하다.
  단 **Linux 에서 그 repo 를 클론해 Claude Code 를 돌리면 소문자 `claude.md` 는 로드되지 않는다.**
  그럴 가능성이 있는 repo 는 `CLAUDE.md` 로 만든다.
- **남과 공유하는 repo 라 `claude.md` 를 두기 어색하면**: 그 판의 관례 파일(`AGENTS.md` 등)을 만들고,
  `CLAUDE.md` 에는 `@AGENTS.md` 한 줄만 넣어 import 한다. 아래에 Claude 전용 지시를 덧붙여도 된다.
- **개인 전용·커밋하지 않을 내용**은 `CLAUDE.local.md` 에 두고 `.gitignore` 에 넣는다.

**확인 방법**: 세션에서 `/context` 를 치면 **Memory files** 목록에 실제로 로드된 파일이 나온다.
`/memory` 는 위치를 나열하고 편집기로 열어준다. 로드 여부는 짐작하지 말고 이 둘로 확인한다.
## 1. 파일 네이밍 — GNU 스타일
...
### 2.2.1 다른 프로젝트의 `.vcxproj` 는 직접 고치지 말고 "무엇을 추가해야 하는지"만 알려라 (강제)

Common 모듈을 고쳐서 **그 모듈을 쓰는 다른 프로젝트에 파일 추가가 필요해졌을 때**, Claude 가 그 프로젝트들의
`.vcxproj`/`.filters` 를 알아서 고치지 않는다. **어느 프로젝트에 어느 모듈(cpp/h)이 더 필요한지 목록으로 알리는
것까지가 Claude 의 일**이고, 실제 추가는 사용자가 한다.

- 알릴 것: 영향받는 프로젝트 경로, 추가해야 할 모듈 파일, 이유(어떤 include 가 새로 생겼는지).
- 현재 작업 중인 프로젝트 자신의 `.vcxproj` 는 이 규칙 대상이 아니다 — §2.2 대로 넣는다.

**Why:** 사용자 명시 (2026-09-10, CSCListBox 에 툴팁을 넣으며 `SCToolTipCtrl.h` 의존이 생겨 5개 프로젝트의
링크가 깨지게 된 건). *"어떤 기능 개선을 위해 추가적인 파일 추가정도는 내가 그냥 빌드해보면 바로 알수있으니
미리 이렇게 알아서 넣어줄것까지는 없다. 또 가끔은 클로드도 실수할때도 있으므로 (…) 다음에는 그냥 어떤 모듈들이
프로젝트에 더 추가되어야 한다 정도만 알려주면 된다."* 빌드 한 번이면 드러나는 문제를, 여러 repo 를 건드리는
변경으로 미리 막을 이유가 없다. 그쪽이 오히려 위험하다 — 실수가 여러 repo 에 흩어진다.

**단, 영향 자체는 반드시 미리 알린다** (§5C.1). "알아서 고치지 말라" 는 것이지 "말하지 말라" 가 아니다.
...
### 2.3.1 Common 을 고치기 전에 — "이미 잘 쓰는 곳"이 어떻게 쓰는지부터 확인하라 (강제)

§2.3 이 "모듈 고유 문제는 모듈에서 고쳐라" 라면, 이건 그 **앞 단계** 다. 증상을 보고 공유 모듈이 부족하다고
판단되면, **고치기 전에 그 모듈을 이미 문제없이 쓰고 있는 곳을 찾아 무엇이 다른지 확인한다.**
잘 쓰는 곳이 하나라도 있으면 결함은 모듈이 아니라 **내 사용법에 있을 확률이 압도적으로 높다.**

**의무 절차 — Common 에 코드를 넣거나 고치기로 마음먹은 순간:**

1. 그 모듈을 쓰면서 같은 증상이 없는 파생/사용처를 찾는다. (`grep` 으로 상속·호출처)
2. **그 코드를 열어 내 호출과 한 줄씩 대조한다.** 어떤 API 를 쓰는지, 순서가 어떤지.
3. 다르면 **내 쪽을 고친다.** 모듈은 건드리지 않는다.
4. 정말 모듈 결함이면, 그때 (a) 왜 기존 사용처들은 멀쩡한지 (b) 고쳤을 때 그 사용처들이 어떻게
   달라지는지를 **먼저 말하고** 진행한다.

**Why:** 2026-09-06 MiniClock2 `CTimeListDlg` 를 `CSCThemeDlg` 파생으로 옮기던 중, 창 둘레에 OS 기본
밝은 테두리가 남았다. 원인을 `CSCThemeDlg::set_back_color` 가 DWM border 를 동기화하지 않는 결함으로
보고 그 함수에 동기화 코드를 넣으려 했다. 사용자가 막았다 — *"이미 bookmarkedit dlg에서는 이런 밝은
테두리가 안그려지는걸 봐서는 SCThemeDlg를 고칠일이 아니지 않나? (…) 왜 자꾸 이미 잘 동작하고 있는
SCThemeDlg를 고치려하나? 그렇게 고쳤을때 다른 파생 클래스들에도 영향이 갈수도 있는데. 자꾸 뭔가를
넣으려하지말고 어떻게 구현해서 이슈화가 되지 않았는지를 파악하라."* 실제로 `CBookmarkEditDlg` 는
`set_back_color` 를 **아예 쓰지 않고** `CSCThemeDlg::set_color_theme` 한 경로만 탄다. 그 안에서 색 복사와
DWM border 동기화가 함께 일어난다. 즉 모듈은 정상이었고, §2.1("응용단은 `set_color_theme` 한 줄만,
개별 색 setter 금지")을 내가 어긴 것이 원인이었다.

**이 실수가 특히 위험한 이유** (사용자 지적) — *"너도 실수할수 있는데 그렇게 실수해놓고 마치 기존 코드가
부족한 듯 막 코드를 넣게되면 이게 나중엔 어떻게 되겠는가? 진짜 이런 실수는 매우 심각하다."*
내 오용을 덮으려고 넣은 코드는 **N 개 프로젝트가 공유하는 자산에 영구히 남는다.** 왜 있는지 아무도
모르고, 지우면 뭐가 깨질지 몰라 못 지우며, 그런 것이 쌓이면 진짜 필요한 코드와 구분이 안 된다.
응용단 실수는 그 파일 하나로 끝나지만 Common 실수는 끝나지 않는다.

### 2.4 같은 일을 이미 한 프로젝트가 있으면 그것을 본으로 삼는다 (강제)

새 프로젝트에 어떤 구조·패턴을 적용할 때, **같은 일이 이미 적용된 프로젝트가 있으면 반드시 그 파일을 열어
구조를 그대로 따른다.** 처음부터 다시 설계하지 말 것. "동작은 하지만 형태가 다른" 결과가 곧 불일치이고,
나중에 둘을 비교할 때 어느 쪽이 기준인지 알 수 없게 된다.

**의무 절차:**

1. 적용 전에 같은 구조를 가진 기존 프로젝트를 찾는다 (문서의 적용 현황, 없으면 grep).
2. 그 파일을 **열어서 읽는다.** 기억이나 요약이 아니라 실제 파일.
3. 블록 구성·분기 위치·매크로 이름·주석 배치까지 그대로 따르고 값만 바꾼다.
4. **일부러 다르게 할 이유가 있으면 그 이유를 보고에 명시한다.** 말없이 다르게 만들지 말 것.
5. 작업 후 두 파일을 대조해 구조가 같은지 확인한다.

**Why:** 2026-09-01, LMMLoginManager 에 `version.h` 를 새로 만들면서 **같은 세션에서 이미 읽었던**
LMMHost 의 `version.h` 구조를 따르지 않고 새로 설계했다. LMMHost 는 버전 숫자 전체를 제품 분기
(`#ifdef`/`#else`) 안에 두는데 새로 만든 쪽은 분기 밖에 한 벌만 뒀다. 두 빌드가 나란히 설치되어 따로
배포되므로, 한쪽 버전을 올리면 다른 쪽이 따라 움직이는 구조였다. 사용자 지적 — *"LMMHost는 그렇게
작업해놓고 왜 LMMLoginManager는 그렇게 작업하지 않았나? 일관성을 무시하는 건가?"*, *"이런걸 놓치나?"*
참조 구현을 같은 세션에서 눈으로 보고도 놓쳤다는 것이 문제의 핵심이다.

**적용 범위:** `version.h` 같은 구성 파일뿐 아니라 클래스 골격, 분기 매크로 배치, 프로젝트 등록 방식,
로그 태그 형식 등 "이미 어딘가에 한 번 한 것" 전부.
...
## 2A.1 오해할 수 있는 함수 이름은 명확히 — 이름이 곧 계약 (강제)

함수 이름이 **실제로 하는 일보다 넓게 읽히면** 이름을 좁힌다. "이 이름만 보고 다른 뜻으로 읽을 사람이 있나?" 가 판정 기준이고, 있으면 이름에 *한정어* 를 붙인다.

- 특정 source 에서만 값을 얻는다면 그 source 를 이름에 넣는다: `get_frame_rate()` → **`get_frame_rate_from_media_info()`**
- 특정 경로/상태에서만 유효하면 그 사실이 이름이나 접근 지정자에 드러나야 한다.
- 같은 개념에 접근자가 둘 이상이면 **하나만 public** 으로 두고 나머지는 private 로 내린다. 이름을 고치는 것만으로는 부족하다 — 부를 수 있으면 언젠가 부른다.

**Why (2026-08-23, Endorphin2):** `CDShow::get_frame_rate()` 는 이름상 "영상 fps 를 구하는 함수" 로 읽히지만 실제로는 *MediaInfo 출력 문자열을 파싱* 하는 LAV 경로 전용 내부 함수였다. 내장 FFmpeg 경로에서는 그 문자열이 비어 0 을 반환한다. 경로 무관 공개 접근자는 따로 `get_video_fps()` 가 있었는데, 둘이 public 에 나란히 있어 Claude 가 프레임 번호 이동 기능에서 전자를 호출 → fps=0 → 기능 자체가 동작하지 않았다. 사용자 지적 — *"같은 의미의 함수인데 왜 2개가 있는것인가? 하나로 통일시켜야 이런 실수를 안하지 않을까?"*, *"이름만 봐서는 get_frame_rate()이 함수는 분명 비디오의 fps를 구하는 함수이름 아닌가. 오해할만한 함수 이름은 명확히 하는걸 선호한다."*

**How to apply:**
1. 새 함수 이름을 짓거나 기존 함수를 읽을 때 자문 — *이름이 약속하는 범위와 구현이 실제로 보장하는 범위가 같은가?*
2. 다르면 (a) 이름을 좁히고 (b) 호출 가능 범위(public/private)도 함께 좁힌다. 둘 다 해야 한다.
3. 헤더 주석(§2F.1)에 어느 경로/조건에서 유효한지와 **대신 쓸 공개 접근자**를 적는다.
4. 같은 개념의 접근자가 여러 개면 "단일 공개 접근자" 를 헤더에 명시한다.
...
### 2C.0.1 요청하지 않은 기능·UX 추가 금지 (강제)

§2C.0 이 "사용자가 명시한 *동작* 에 제약을 덧붙이지 말 것" 이라면, 이 규칙은 "**사용자가 말하지 않은 *기능* 을 만들지 말 것**" 이다. Claude 가 "있으면 편하겠지" 로 판단해 넣은 UI·상호작용·표시 요소가 전부 여기 해당한다.

**X — 절대 금지:**
- 선택 상태 + 하이라이트 표시 (사용자가 "선택 기능" 을 요청하지 않았는데 추가)
- Enter/Esc 차단, 확인 대화상자 등 "실수 방지" 장치
- 색상 구분, 부가 안내 문구, 툴팁, 진행 표시
- 요청 범위 밖의 getter·상태 노출 API

**Why this rule:** 2026-08-11, Test_ONNX 의 유사도 비교 다이얼로그 구현 시 Ctrl+V 대상 칸을 정하려고 "클릭으로 칸 선택 + 파란 테두리 표시" 를 임의로 추가. 그 `set_round()` 호출이 `CSCStatic::OnPaint` 의 `m_round == 0` 조건을 깨서 `set_back_image(bitmap, DimGray)` 의 배경색이 무시되는 버그를 만들었다. 사용자 지적 — *"쓸데없는 기능을 넣으니 이런 부작용이 나타난다. 이것저것 나름 끌로드가 뭔가를 생각해서 넣은 코드들은 어느 사용자에게는 매우 쓰레기가 될 수 있다."* 요청하지 않은 기능은 가치가 0 이거나 음수인데 부작용은 실재한다.

**적용:**
1. 요구사항에 없는 기능은 **구현하지 않는다**. 필요해 보이면 구현 대신 한 줄로 제안만 한다.
2. 기능이 성립하려면 반드시 있어야 하는 것(예: 메뉴 항목이 동작하려면 팝업을 띄우는 코드)은 추가한다. "있으면 편한 것" 과 "없으면 동작하지 않는 것" 을 구분할 것.
3. 애매하면 넣지 않는 쪽. 빠진 것은 나중에 추가하면 되지만, 넣은 것은 사용자가 발견해야 지울 수 있다.
4. 불가피하게 요청 밖 요소를 넣었다면 **그 사실을 응답에 명시**한다. 조용히 끼워넣지 말 것.

### 2C.0.2 언급이 없는 것을 미해결로 단정하지 말 것 (강제)

§2C.0.1 이 "요청하지 않은 *기능* 을 만들지 말라" 면, 이건 "**말이 없는 것을 아직 안 고쳐진 것으로 읽지 말라**" 다.
지적했던 문제를 사용자가 더 이상 언급하지 않으면 그건 대개 **해결됐다는 뜻**이다. 확답을 못 들었다는 이유로
미해결로 가정하고 다시 손대면, 잘 동작하던 코드를 도로 망가뜨린다.

**침묵은 신호가 아니다.** 언급이 없는 이유는 여러 가지다 — (1) 해결돼서, (2) 남아 있지만 더 급한 것이 먼저라서,
(3) 아직 확인을 못 해서. 어느 쪽인지 추측으로 정하지 말 것.

**적용:**
1. 해결 여부를 알아야 다음 작업을 정할 수 있으면 **한 줄로 묻는다.** 짐작해서 재수정하지 않는다.
2. 몰라도 되면 **건드리지 않는다.** 마지막으로 확인된 상태를 그대로 둔다.
3. 다른 요구사항(예: 그림자 복구) 때문에 *이미 해결된 것이 되살아나는* 구조라면, 고치기 전에
   **그 인과를 먼저 알린다.** — "A 를 되살리면 B 가 같이 돌아옵니다" 를 말하고 진행.
4. 사용자가 화제를 바꿨다면 그 화제를 하라. 이전 항목의 재점검은 요청이 있을 때.

**Why:** 2026-09-04 SCDeskTools 캡처 노트. 상단 1px 회색선을 없앤 뒤 사용자가 그 얘기를 하지 않고
오프셋·그림자로 넘어갔는데, Claude 는 "확답이 없으니 아직 남아 있다" 고 보고 그 자리를 다시 손댔다.
사용자 지적 — *"만약 그 문제가 여전히 남아있었다면 계속 그 문제를 얘기했겠지. 설령 여전히 남아 있었어도
다른 더 급한 기능을 수정요청할수도 있을테고. 그걸 아직 안고쳤다고 무조건 판단하면 안되지. 언급하지 않은건
추측으로 하면 위험하지. 나는 해결됐다고 생각해서 더 이상 얘기하지 않은건데 너는 확답을 못들어서
아직 해결되지 않은줄알고 잘 고쳐진걸 또 망칠것 아닌가?"*
...
## 5-1. 플랫폼(x86/x64) — 프로젝트당 하나만 실제로 쓴다 (모든 프로젝트)

사용자의 모든 프로젝트는 **x86(Win32) 또는 x64 중 한쪽만** 실제로 사용한다. 둘 다 지원하는 것을 의도한 제품은 없다. `.vcxproj` 에 두 플랫폼이 다 선언되어 있어도 나머지 한쪽은 잔재이며, 빌드가 되는지조차 보장되지 않는다.

**적용:**
- 빌드·검증은 **현재 선택된 플랫폼 하나**만 하면 된다. `.suo` 의 `ActiveCfg` 가 `Debug_HelpU|x86` 이면 `Win32` 만 빌드한다.
- 반대쪽 플랫폼이 빌드 실패해도 그것만으로 회귀가 아니다. 원래 안 되던 구성일 수 있으므로 **먼저 사용 여부를 확인**하고, 사용자에게 보고만 한다.
- 구성별 설정(PreBuildEvent, 매크로 등)을 넣을 때는 그래도 **선언된 전 구성에 일관되게** 넣는다 — 나중에 플랫폼을 갈아탈 때 한쪽만 누락되면 원인 찾기 어렵다.

**Why:** 사용자 명시 (2026-07-29, KoinoCheckTool). *"내 모든 프로젝트는 x86 / x64중에 1개만 사용한다. 둘 다 지원하는 의도의 제품은 없다. 그러니 x86이 선택되어 있으면 그냥 x86만 하면 된다."* §5B-0 의 "구성을 확인하지 않고 빌드" 지적과 짝 — 그쪽이 *어느 구성인지 확인하라*, 이쪽이 *확인했으면 그 하나만 하면 된다*.
...
> 빌드를 위한 프로세스 강제 종료(§5B-0.1)도 하지 않는다. push 지시를 받았을 때도 빌드하지 않는다.
>
> **빌드해도 되는 유일한 경우**: 사용자가 그 세션에서 명시적으로 시켰을 때 — *"빌드해봐라"*,
> *"네가 직접 빌드하고 버그 수정까지 완료해라"* 등. 그때는 그 요청 범위 안에서만 빌드·디버깅·테스트한다.
> **별 말이 없으면 빌드하지 않는다.** "확인차", "안전하게", "컴파일만" 도 안 된다.
>
> **Why:** 사용자는 VS 에서 F7(빌드) / F5(실행) 로 즉시 하므로 **사용자가 하는 편이 압도적으로 빠르다.**
> Claude 의 외부 MSBuild 는 (a) 매번 수십 초를 쓰고, (b) 링크를 위해 실행 중인 exe 를 강제 종료해
> **사용자가 띄워놓고 테스트하던 앱을 끊으며**, (c) 산출물을 실제 설치 경로에 덮어쓰기도 하고,
> (d) **사용자가 실제로 쓰는 구성과 다른 구성으로 빌드하기 일쑤다.** 활성 구성은 `.suo` 의 `ActiveCfg` 를
> 읽으면 바로 알 수 있고(§5-1) 실제로 그렇게 확인한 세션도 있다. **어려워서가 아니라 Claude 가 확인을
> 건너뛰고 짐작으로 고르기 때문이다.** `.vcxproj` 에는 안 쓰는 구성이 잔재로 남아 있어 짐작이 자주
> 빗나가고, 그 구성에서만 나는 에러를 원인이라 착각해 시간을 쓴다
> (2026-07-21 LMMHost — 배포 구성 `Release_Lmm10`(v140_xp) 대신 미사용 `Release`(v145)로 빌드해
> `d2d1effects_2.h` C1083 을 좇음). 사용자는 F7 하나로 항상 자기 구성으로 빌드한다.
> 사용자 지적 (2026-08-28) — *"내가 F7 누르면 바로 빌드되고 F5 눌러서 바로 실행해서 테스트 한건데
> 뭐하러 너는 맨날 시간 낭비하면서 빌드하는지 납득이 가질 않는다. (…) 별 다른 말이 없다면
...
수정 요청을 받아 코드를 고친 뒤 빌드하려는데 산출물(exe/dll)을 물고 있는 프로세스가 실행 중이면, **사용자에게 종료를 요청하지 말고 그냥 강제 종료한 뒤 빌드까지 끝낸다.**

- 대상: 방금 수정한 코드의 산출물을 잠그고 있는 프로세스 (예: `KoinoHost.exe`, `KoinoViewer.exe`, `RemoteSDKDemo.exe`, 테스트 앱 등).
- 예외: 사용자가 **"이건 강제종료시키지 마라"** 라고 명시한 경우에만 멈추고 묻는다.
- 종료했다는 사실은 결과 보고에 한 줄 적으면 충분하다. 사전 확인은 불필요.

**Why:** 사용자 명시 (2026-08-06, KoinoRemoteSDK_v10). *"내가 클로드에게 어떤 수정요청을 한 경우 클로드가 수정한 후 빌드까지 해야하는데 만약 실행중이라면 그냥 강제시키고 빌드해도 전혀 관계없다. 왜? 수정요청을 했으니. 그러니 특별히 내가 '이건 강제종료시키지 마라'라고 말하지 않는 이상 그냥 강제종료시키고 빌드까지 하는 것을 기본으로 하라. 이거 매번 말하기도 귀찮다."* 수정을 요청한 시점에 그 앱을 재시작할 의사는 이미 포함되어 있다. LNK1104/LNK1168 을 만날 때마다 종료를 요청하면 왕복만 늘어난다 (§5B-0 의 2026-07-16·07-21·07-29 반복 지적과 같은 비용).

**세 번째 반복 지적 (2026-07-29, ASee)** — SVG 지원 크래시 수정 후, 디버그 세션이 exe 를 잡아 `LNK1168` 이 난 상태에서 "빌드하면 됩니다/테스트해달라"로 넘겼다. 사용자 지적 — *"다음부터는 수정 후 빌드까지 완료하고 테스트 요청하라."* 재확인된 절차: 수정 → 전체 빌드 시도 → **링크가 exe 잠금으로 막히면 그 사실을 먼저 알리고, 잠금 해제(프로세스/디버그 종료)를 확보한 뒤 링크까지 성공시킨 다음** 테스트 요청. "컴파일 통과"는 완료가 아니다.
...
### 5C.1 전제(precondition)를 바꾸는 변경 — 그 전제에 기대던 코드를 전부 찾아 제시하라 (강제)

§5C 가 "함수·동작을 바꿀 때 호출처를 보라" 라면, 이건 그보다 위험한 종류다. **매니페스트 설정,
컴파일 옵션, 전역 정책, 베이스 클래스 동작, 데이터 포맷** 처럼 *코드가 서 있는 바닥* 을 바꾸면
호출 관계가 없는 코드까지 조용히 틀려진다. grep 으로 호출처를 따라가는 것만으로는 절대 안 잡힌다.

**의무 절차 — 전제를 바꾸기로 한 순간:**

1. 그 전제가 **무엇을 보장하고 있었는지** 한 문장으로 적는다.
   (예: "DPI-unaware 라 OS 가 앱 출력을 균일 확대해 준다 → 96 DPI 기준 픽셀을 박아도 어느 배율에서든 맞다.")
2. 그 보장에 **기대고 있던 코드를 전부 찾는다.** 호출 관계가 아니라 *의미* 로 찾아야 한다 —
   하드코딩된 숫자, 고정 상수, "항상 96", "항상 1:1", "항상 이 포맷" 을 전제한 계산 전부.
3. **바뀐 전제 아래에서 각각이 여전히 맞는지 판정**하고, 틀려지는 것을 목록으로 만든다.
4. 그 목록을 **구현 전에 사용자에게 제시**한다. 같이 고칠지, 나눠 갈지는 사용자가 정한다.
5. 고치지 않고 남기기로 한 것은 그 자리에 주석으로 남긴다 — 다음 사람이 또 밟는다.

**"작성 당시에는 맞았다" 는 면책이 되지 않는다.** 틀리게 만든 것이 이 변경이므로, 틀려진 시점에
제시할 책임도 이 변경에 있다.

**Why:** 2026-06-29 SCDeskTools `29a266a`. 고DPI 모니터에서 캡처 오버레이가 확대·흐려지는 문제를
고치려고 매니페스트를 DPI-unaware → Per-Monitor V2 로 바꿨다. 그 순간 스케일링 책임이 OS 에서
앱으로 넘어왔는데, 커밋은 **오버레이만** 보고 끝냈다. 메인 다이얼로그 레이아웃(창 크기 400x288,
margin 14, 버튼 높이 50…), 돋보기 창 크기, 캡처 노트 리사이즈 여백, 줄자·각도기 핸들 반경,
줄자의 `const double dpi = 96.0` cm 환산 — 96 DPI 를 전제로 박아둔 상수가 프로젝트 전반에 있었고
전부 조용히 틀려졌다. 두 달 뒤 사용자가 175% 로 실행해 보고서야 드러났다.
사용자 지적 (2026-09-04) — *"정책이 바뀌었다면 관련 코드들을 이미 그때 제시했어야 했다.
A를 고쳤을 때 어디가 영향을 받을지 또한 당연히 제시하고 수정했어야 하는게 아닌가?
이 규칙은 정말 중요하다. 반드시 제시해야 한다. 그때는 맞았으니 문제없다?
그럼 틀리게 된 상황에서는 제시했었어야지!"*

**전제를 바꾸는 변경의 예 (이 목록을 보면 바로 §5C.1 을 적용한다):**
매니페스트(DPI 인식·UAC·공용 컨트롤 버전) · 컴파일러 charset 옵션 · 툴셋/SDK 변경 ·
좌표계·단위 변경(논리↔물리 픽셀, 0-base↔1-base) · 파일/레지스트리 저장 포맷 ·
베이스 클래스의 기본 동작 · 스레드 모델 · 전역 색·폰트 정책.

### 5C.2 원인은 측정으로 확정한 뒤 고친다 — 추측으로 대량 수정 착수 금지 (강제)

증상을 보고 원인을 *추론* 했다면, 그건 아직 가설이다. **가설 상태로 코드를 고치기 시작하지 말 것.**
특히 공유 모듈(Common)·전역 설정·매니페스트처럼 되돌리기 비싼 대상은 측정 전 착수 금지.

**추측 자체는 금지가 아니다.** 사용자 정리 (2026-09-04) — *"AI도 실수는 할 수 있다. 다만, 추측만으로
작업하지 말것이며 그래서 근거를 대라고 내가 요구하는 것이며 실제 부작용을 보여주면 나도 이해하고
기다릴 수 있다. 그러니 추측은 하되 그 추측이 정확한지를 확인하는 단계까지 검증하고 진행하자는 것이다."*
즉 금지되는 것은 추측이 아니라 **검증 단계를 건너뛰는 것**이다. 검증에 드는 시간은 사용자가 기다려 준다.
틀린 추측으로 고쳤다가 되돌리는 비용이 훨씬 크기 때문이다.

**의무 절차:**

1. 가설을 한 문장으로 적는다. 그리고 **"이 가설이 맞다면 무엇이 관측되어야 하는가"** 를 같이 적는다.
   관측 가능한 예측이 안 나오면 그건 가설이 아니라 느낌이다.
2. 그 관측을 **실제로 한다.** 값 조회, 로그, 화면 캡처 — 코드를 고치기 전에.
3. 측정과 가설이 다르면 **가설을 버린다.** 측정을 의심해서 밀어붙이지 말 것.
4. 고친 뒤에도 **같은 방법으로 다시 측정해** 고쳐졌음을 확인한다. "고쳤을 것이다" 로 넘기지 않는다.
5. 한 번에 하나씩. 여러 곳을 동시에 바꾸면 무엇이 효과였는지 알 수 없고, 되돌릴 때도 통째로 되돌아간다.

**측정 도구 자체도 의심 대상이다.** 도구가 대상과 다른 조건에서 돌면 멀쩡한 것을 고장으로 읽는다.
2026-09-04 실제 사례 — 창 크기를 재는 PowerShell 스크립트가 DPI-unaware 라 Windows 가 값을 가상화해
700x504 를 400x288 로 돌려줬다. **정상 동작을 두 번이나 고장으로 오판하고, 안 고쳐도 될 것을 고쳤다.**
측정이 이상하면 대상보다 측정 조건을 먼저 확인한다.

**Why:** 2026-09-04 SCDeskTools DPI 작업. 고DPI 에서 오버레이가 어긋난다고 보고 `CSCD2Context` 의
D2D 타겟 DPI 설정을 뜯어고치려 했다. 근거는 "desktop 윈도우가 PER_MONITOR_AWARE 로 보고되니
`GetDpiForWindow(GetDesktopWindow())` 도 배율을 따라갈 것" 이라는 추론 하나였다.
그 모듈은 ASee·Endorphin2 등이 공유한다. 사용자가 *"잠깐. 수정전에 그 취약점을 확인해볼 수 있나?"* 라고
막아 세워 실제로 175% 에서 재보니 **그 호출은 175% 에서도 96 을 돌려줬다** — 취약점 자체가 없었다.
사용자 지적 — *"잘못된 추측으로 인해 앱 전체의 구조가 크게 변경되고 또 이를 나중에 다시 번복할까봐
걱정되는 것이다. (…) 이런 시행착오를 줄이기 위해서라도 단계별 확인후에 작업을 하던가."*,
*"내가 실제 부작용을 보자고 안했으면 너는 네가 추측한대로 앱을 뒤집어놨을듯하다."*

같은 날 이어진 교훈 하나 더 — **증상이 A 영역에 보여도 원인이 A 에 있다는 보장은 없다.**
캡션바가 잘리는 문제로 클라이언트 레이아웃 코드를 세 번 고쳤는데, 실제 원인은 매니페스트가
Per-Monitor **V1** 이라 비클라이언트 자동 스케일링이 없었던 것이었다. **환경·설정·빌드 산출물을
먼저 확인했으면 첫 판에 끝났다.** 코드부터 열지 말고 그 코드가 서 있는 바닥부터 볼 것 (§5C.1).

### 5C.3 참조 구현에 맞출 때 — 기준은 "참조에 있느냐" 가 아니라 "지금 모습이 유지되느냐" (강제)

§2.4 에 따라 다른 프로젝트의 참조 구현을 본으로 삼아 고칠 때, **현재 사용자가 보고 있는 외관·동작은
그대로 유지되어야 한다.** 참조 쪽에 없다는 이유로 빼서 지금 모습이 달라지면 그건 이식이 아니라 회귀다.

**판정 기준 — 넣을지 말지 정하기 직전 자문:**
> "이걸 안 넣으면 지금 화면이 달라질 가능성이 있나?"
> 조금이라도 있으면 **넣는다.** 참조 구현에 없어도, 요청받지 않았어도 넣는다.

**"지금은 OS 기본값이라 어차피 그렇게 보인다" 는 유지의 근거가 되지 않는다.** 내가 통제하지 않는
기본값에 외관을 걸어두는 것 자체가 위험하다 — 특히 그 주변(프레임·NC·스타일·DPI)을 고치는 중이라면
그 변경이 기본값의 적용 조건을 건드릴 수 있다. **명시 호출로 고정하라.**

**§2C.0.1("요청하지 않은 기능 추가 금지")과 충돌하지 않는다.** 그쪽은 *새 기능·새 UX* 를 만들지 말라는
것이고, 이쪽은 *이미 있는 모습을 유지* 하라는 것이다. 유지에 필요한 코드는 "추가 기능" 이 아니다.
헷갈리면 이렇게 나눈다 — **사용자가 지금 보고 있는 것 = 유지(넣는다). 지금 없는 것 = 만들지 않는다.**

**Why:** 2026-09-06 MiniClock2 `CTimeListDlg`. borderless 다이얼로그의 흰 띠를 고치며 Endorphin2Dlg /
SCCapturedNoteDlg 를 본으로 NC 처리를 정비하는 중, `set_window_corner_round` 를 "SCCapturedNoteDlg 에는
없고 흰 띠와 무관한 외관 변경" 이라는 이유로 넣지 않았다. 그런데 그 창은 이미 둥근 모서리로 보이고
있었다(Win11 의 `DWMWCP_DEFAULT` 기본 동작). 사용자 지적 — *"이미 라운드로 표시되고 있지 않나?
그런데 그걸 SCCapturedNoteDlg 에는 없다는 이유로 뺐나? 그게 뺄 근거가 되나?"*, *"뭔가를 안넣으면
라운드가 사라진다면 (…) 호출해서라도 원래의 모습은 그대로 유지됐어야 한다는 말이다."*
프레임/NC 계산을 바꾸는 작업이었으므로 기본값에 맡긴 것 자체가 실제 위험이기도 했다.

### 5C.4 근거 없는 단순 가정을 알고리즘의 토대로 삼지 말 것 (강제)

§5C.2 가 "원인은 측정으로 확정하라" 면, 이건 그 앞이다. **알고리즘의 핵심 판정을 "대충 이럴 것이다"
수준의 가정 위에 세우지 말 것.** 특히 "가장 많은 것이 X 다", "보통 이게 더 크다", "대개 이 방향이다"
같은 다수결·평균 류의 직관은 반례가 흔하다.

**가정을 쓰기 전 자문 — 둘 중 하나를 만족하는가:**

1. **문제의 구조에서 필연적으로 따라 나오는가.** (예: 단어 상자 *바깥* 은 정의상 그 단어가 아니다.)
2. **실제 데이터로 검증했는가.** 합성 예제 하나가 아니라, 실패할 만한 경우를 일부러 포함해서.

둘 다 아니면 그 가정은 쓰지 않는다. 대안이 없으면 **가정을 세우는 대신 실제로 여러 후보를 넣어 보고
결과로 고른다.** 계산으로 알아낼 수 없는 것을 계산으로 알아낸 척하지 말 것.

**Why (2026-09-12, SCDeskTools OCR):** 두 번 연속 같은 실수를 했다.

- *"단어 상자 안에서 가장 많은 색 = 배경"* — 잉크가 차지하는 면적 비율에는 상한이 없다. 굵은 글자는
  당연히 50% 를 넘는다. `150매` 같은 큰 글자에서 글자색과 배경색이 통째로 뒤바뀌었다.
  데이터를 보기도 전에 기하학만으로 반증되는 가정이었다.
- *"검출된 단어 높이의 중앙값 = 그 이미지의 글자 크기"* — 엔진이 **못 찾은** 글자는 중앙값에 잡히지
  않는다. 큰 제목만 검출된 사진에서 중앙값이 44 로 나와 이미지를 x0.59 로 *줄였고*, 그래서 작은 글자를
  아예 뭉갰다. 정답은 x1.3~x2 로 *키우는* 것이었다.

사용자 지적 — *"이러한 단순한 가정은 절대 위험하고 AI답지 않다. 이런 말도안되는 가설은 다음부터는
절대 사용하지 마라."*

**두 경우 모두 고친 방법이 같다.** 추정을 버리고, (1) 구조적으로 참인 근거로 바꾸거나
(상자 바깥 = 배경), (2) 후보를 실제로 넣어 보고 결과로 골랐다 (배율·방향 스윕).

**보고 의무:** 불가피하게 가정을 썼다면 코드 주석과 보고에 *그것이 가정이라는 사실과 깨지는 조건* 을
함께 적는다. 조용히 끼워 넣지 말 것.
...
## 5E. LMM 1.0 작업 중 3.0 SE 를 같이 고치지 말 것 (강제)

LinkMeMine **1.0** 제품(LMMLoginManager, LMMHost, LMMViewer, ManualLauncher 등)을 수정할 때, **3.0 SE 에도 같은 수정을 적용하지 않는다.** 그리고 **"3.0 SE 도 같이 고칠까요?" 라고 묻지도 않는다.**

- 수정이 간단하든 복잡하든 동일하다. 질문 자체를 하지 않는다.
- 제안도 하지 않는다. "SE 에도 같은 문제가 있어 보인다" 같은 언급으로 사용자가 판단하게 만들지 말 것.
- SE 를 고치는 것은 **사용자가 SE 를 명시적으로 지목해 지시할 때뿐**이다.
- **유일한 예외**: 보안 취약점·데이터 손상·서비스 중단급의 *아주 치명적인* 결함. 이때도 **고치지 말고 보고만** 한다. 판단은 사용자가 한다.

**Why (2026-08-21):** 과거 세션들이 1.0 작업 중 "3.0 SE 도 동일하게 수정할까요?" 를 물었고, 사용자는 작업이 커 보이지 않으면 승인했다. 문제는 **사용자가 SE 를 매번 테스트하지 않는다**는 것이다. 그래서 SE 쪽 변경은 검증되지 않은 채 잠복하다가 한참 뒤에 터진다.

실제 사고 — 어느 세션이 SE 의 `viewer-core.vcxproj` / `KNORemoteCoreLibrary.vcxproj` 에 존재하지 않는 절대경로(`D:\1.Projects_C++\Common\CommonLib.props`)를 `Condition` 없이 Import 로 박아 넣었다. 그 상태로 커밋되지 않고 방치되다가, 2026-08-21 사용자가 다른 솔루션 배포 확인을 위해 SE 솔루션을 열었을 때 **프로젝트가 아예 로드되지 않아** 원인 추적 → 미커밋분 보존 → 마지막 push 지점으로 원복 → 재구성까지 반나절이 들었다. 사용자 지적 — *"간단하든, 복잡하든 3.0도 같이 고칠까요?라는 질문은 하지마라."*

또한 **9월에 1.0 과 3.0 SE 를 단일 소스로 통합하고 빌드 구성만 달리할 계획**이다. 지금 SE 를 따라 고치는 작업은 대부분 통합 시점에 폐기되거나 다시 정리된다. 지금 하는 일이 두 번 일이 되고, 그 사이에 검증되지 않은 변경이 쌓인다.

**적용 범위**: 1.0 ↔ 3.0 SE 사이의 "따라 고치기" 전반. 반대 방향(SE 작업 중 1.0 을 같이 고치는 것)도 동일하게 금지한다.
...
4. **"push" 명령 = Common 포함 모든 repo push 가 원칙 (강제, 2026-06-04 사용자 정정)** — 사용자가 "push" / "모두 푸시하라" 등 push 를 지시하면 그 명령의 범위는 *변경이 있는 모든 repo* 다. 현재 프로젝트 repo 뿐 아니라 **Common 도 함께 push** 한다. Common 에 *이번 세션에서 만들지 않은 타 세션 변경분* 이 있어도 포함한다 (§5 의 sync commit 으로 분리). 사용자 명시 — *"앞으로 푸시하라고 하면 Common까지 모두 푸시하는 것을 원칙으로 한다"* + *"'해당 프로젝트가 알아서 push' 의 해당 프로젝트는 이 프로젝트도 되고 다른 프로젝트도 될 수 있다"* (= 현재 세션이 Common 의 타 세션 변경분을 push 해도 됨).
   ...
   - **push 지시를 받으면 빌드하지 않는다 (강제, 2026-08-28)** — 사용자가 push 를 지시했다는 것은 *이미 빌드하고 기능까지 확인했으며 지적할 것이 없다* 는 뜻이다. 그 시점에 Claude 가 다시 빌드할 이유가 없다. commit·push 만 한다.
     - 특히 빌드 명령은 링크를 위해 실행 중인 exe 를 강제 종료(§5B-0.1)하므로, **사용자가 띄워놓고 보던 앱을 끊는다.** 시간 낭비에 그치지 않는다.
     - 애초에 Claude 는 빌드하지 않는 것이 기본이다 (§5B-0). push 단계는 더더욱 아니다.
     - 사용자 지적 (2026-08-28) — *"푸시하라고 했더니 왜 빌드하냐. 내가 이미 빌드해서 기능까지 확인했고 별 다른 지적사항이 없으니 푸시하라고 한건데 왜 굳이 빌드를 다시 하냐."*
...
## Confluence 문서 생성 — 일반 페이지로 (강제, 모든 프로젝트)

Confluence(`koinodoc.atlassian.net`)에 문서를 만들 때는 **일반 페이지(page)로 생성**한다. 라이브 문서(live doc)로 만들지 말 것.

**How to apply:** `createConfluencePage` 호출 시 `contentType: "page"` 만 지정하고 **`subtype` 은 넘기지 않는다**. 공간/부모 위치, 본문 작성 방식은 동일하다.

**Why:** 사용자 명시(2026-09-04) — *"다음부터는 라이브 문서가 아닌 페이지로 작성하는 것을 원칙으로 하라. 간혹 라이브문서로 하면 뭔가 동기화가 잘 안되는듯하다."* 라이브 문서는 동기화가 간헐적으로 어긋나는 경우가 있어 기본값에서 제외한다.
(이전 방침은 반대였다 — 2026-06-24 *"무조건 페이지가 아닌 라이브문서로 생성하라"*. 운용해 보고 뒤집힌 것이므로 옛 지시를 근거로 되돌리지 말 것. 라이브 문서는 사용자가 그 문서에 대해 명시할 때만.)
...
응답에는 **사용자가 판단하거나 확인해야 하는 것만** 담는다. Claude 의 작업 과정은 화면에 남기지 않는다.

**보여줄 것:** 코드 변경 내용과 변경 파일 · 실측/검증 결과(성공·실패 모두) · 사용자의 결정이 필요한 선택지 · 커밋 해시와 푸시 결과.

**보여주지 말 것:**
- 탐색·확인 과정. 어떤 파일을 왜 읽었는지, 무엇을 grep 했는지.
- **Claude 자신의 실수와 복구 절차** — 되돌림, 재시도, 도구 문법 오류 등. 조용히 고치고 결과만 보고한다.
- 묻지 않은 중간 판단 근거의 나열.

**How to apply:**
- 도구 호출 횟수 자체를 줄인다. 이미 확인한 것을 다시 확인하지 않는다.
- `Bash` 로 `grep ... | head` 파이프라인을 돌리면 명령줄과 출력이 그대로 화면에 찍힌다. 같은 일을 `Grep`/`Read` 전용 도구로 하면 한 줄로 접힌다 — 전용 도구를 쓸 것.
- 근거는 물어보면 그때 펼친다.

**Why:** 사용자 명시(2026-07-24). SCColorTable 분석 세션에서 탐색용 grep 출력과 Claude 의 실수 복구 과정이 화면을 채웠다 — *"이런 내용들이 화면을 계속 스크롤해서 우리가 무슨 대화를 나눴는지 찾기도 힘들다"*, *"네가 뭐 작업하다가 실수해서 되돌리니 마니 그런거까지 뭐하러 다 찍냐"*. 대화 자체를 되짚기 어려워지는 비용이 크다.

## "모두 체크하라" — 5대 공통 점검 항목 (강제, 모든 프로젝트·모든 작업 대상)

사용자가 **"모두 체크하라"** 라고 하면, 대상(함수·클래스·파일·모듈·프로젝트 무엇이든)에 대해 아래 5가지를 **각 항목별로 나눠 상세히** 확인하고 보고한다.

1. **버그는 없는가?** — 논리 오류, 잘못된 인덱스/순서, 죽은 코드, 도달 불가 분기, off-by-one, 부호/오버플로, 미초기화.
2. **예외 처리가 빠진 곳은 없는가?** — 반환값 미확인, 실패 경로 누락, 리소스 해제 누락(핸들·소켓·GDI), 경계값·빈 입력·NULL.
3. **성능상 문제는 없는가?** — 불필요한 동기 대기·busy wait, N 회 반복되는 무거운 호출(레지스트리/윈도우 API/재그리기), 중복 계산, O(N²).
4. **보안상 위험은 없는가?** — 인증서 검증 우회, 검증 없는 다운로드·실행, 경로/명령 인젝션, 버퍼 경계, 하드코딩된 자격증명·내부 주소.
5. **더 읽기 좋은 코드가 있는가?** — 매직넘버, 중복 블록, 네이밍, 함수 분리, 이 저장소의 기존 컨벤션 위반.

**How to apply:**
- 5개 항목을 **번호를 유지한 채 각각 별도 절**로 보고한다. 해당 없으면 "해당 없음"이라고 명시 — 항목 자체를 생략하지 말 것.
- 각 지적은 `파일:줄번호` 로 앵커링하고, **실제로 파일을 읽어 확인한 것만** 적는다 (§5B 검증 없이 단정 표현 금지 적용).
- 성능 지적은 가능하면 측정치를 첨부한다. 측정 못 했으면 "미측정" 이라고 밝힌다.
- 지적만 하고 끝내지 말고 각 항목에 정석 구현 방향을 함께 제시한다.

**Why:** 사용자 명시(2026-07-24). 코드 리뷰·분석 요청 때마다 매번 같은 5가지를 되풀이해 지시하는 비용을 없애기 위해 단축 명령으로 승격.

## 요구사항에 명백히 포함되는 인접 수정은 함께 처리한다

사용자가 특정 함수 하나를 지목해 수정을 요청했더라도, **같은 요구사항을 충족하려면 반드시 함께 고쳐야 하는 다른 지점**을 발견하면 지적만 하고 멈추지 말고 같이 수정한다.

**How to apply:**
- O: 두 곳 모두 수정한 뒤 "A 와 함께, 같은 이유로 B 도 수정함" 이라고 결과 보고.
- X: A 만 수정하고 "B 도 필요해 보입니다" 라고 남겨두기. 사용자가 다시 지시해야 하는 왕복이 생긴다.
- 판단 기준: 그 지점을 고치지 않으면 **요구된 동작이 실제로 완성되지 않는가?** 그렇다면 함께 고친다.
- 예외(그때만 확인): 수정 방식이 두 가지 이상으로 동등하게 갈릴 때, 되돌리기 어려운 파괴적 변경일 때, 명백히 다른 기능 영역일 때.

**Why:** 2026-08-04 KoinoRemoteSDK_v10 / koinohost 작업. `RCTextWnd::ChangeStatusText()` 에 `_KFTC` 접두어("금융결제원 ")를 넣으라는 요청에서, 최초 문구를 세팅하는 `RCTextWnd::init()` 도 같은 리소스 문자열을 쓰는데 고치지 않고 "필요해 보입니다" 라고만 보고 — *"init()에도 당연히 처리해야 하는거 아닌가? 이건 물어볼게 아닌듯한데?"* (§작업 진행 확인 금지 의 연장선)

### 확장 — 작업 중 발견한 사소한 방어 결함도 보고만 하지 말고 그냥 고친다

위 규칙은 "고치지 않으면 요구된 동작이 완성되지 않는가"를 기준으로 삼는다. 그 기준에 미달하더라도, **이미 읽고 있던 코드에서 발견한 최소 방어 결함**은 확인 없이 바로 고친다.

**How to apply:**
- 대상: 널 체크 누락, 형제 코드와의 사소한 불일치처럼 **변경이 국소적이고, 되돌리기 쉽고, 동작 변화가 "크래시 대신 정상" 뿐인** 것.
- O: 고치고 결과 보고에 한 줄. X: "이런 취약점이 있습니다, 원하시면 고치겠습니다".
- 여전히 확인이 필요한 경우: 동작 의미가 갈리는 분기(널일 때 무엇을 할지 정해야 함), 리턴값·에러 규약 변경, 파괴적 변경.
- 판단 기준: *고쳤을 때 사용자가 "왜 이걸 건드렸냐"고 물을 여지가 있는가?* 없으면 그냥 고친다.

**Why:** 2026-08-24 LmmCSManager / Common 작업. `CSCListBox::OnContextMenu` 만 형제 4개 클래스와 달리 `GetParent()` 널 체크 없이 역참조하는 것을 발견하고 "일관성 차원에서 맞추고 싶으시면 말씀해 주십시오" 라고 남김 — *"이러한 취약 코드는 어려운 것도 아니고 최소한의 방어코드이므로 그냥 클로드가 알아서 고쳐도 된다. (…) 확인까지 받아야 할정도로 무슨 부작용이 있을만한 코드도 아니고."*

## 시각적 결과물 — 먼저 최소 구현으로 보여주고, 확정 후에 다듬는다 (강제, 모든 프로젝트)

**"보기 전에는 좋은지 알 수 없는" 작업** — 포커스 표시, 색·간격·애니메이션, 아이콘/레이아웃 등 화면을 봐야 판단이 서는 변경 — 은 **최소 구현 → 스크린샷/빌드로 확인 → 합의 → 그 다음에 다듬기** 순서로 진행한다.

**How to apply:**
- 1차는 **하드코딩으로 충분하다.** 값은 리터럴, 호출부는 한 곳, 새 API·파라미터·enum 없이. 빌드해서 결과를 보여주는 것이 목표다.
- 다음은 **확정 후에** 한다: 공개 setter 추가, 파라미터화(float 전환·비율 노출), 중앙화 헬퍼 추출, 예외 케이스 보정 로직, 문서 주석.
- 1차 구현이 마음에 들면 그때 위 항목들을 얹는다. 마음에 안 들면 버리는 코드가 몇 줄뿐이다.
- 무관한 정리(주석 마커 소급 적용, 리팩터링 등)를 같은 턴에 끼워 넣지 않는다. 체감 소요 시간을 늘리고 결과 확인을 늦춘다.
- 판단 기준: *지금 이 코드가 사용자 마음에 안 들어 버려지면, 버려지는 양이 얼마인가?* 크면 아직 다듬을 때가 아니다.

**Why:** 사용자 명시(2026-08-21). CSCMessageBox 포커스 표시(dot inner line)를 시도하면서, 사용자가 결과를 보기도 전에 `cr_avoid` 대비 보정 루프·`apply_button_focus_style()` 중앙화·public `set_button_focus_style()`·파라미터 float 전환까지 production 품질로 완성했다. *"이렇게 오래걸릴것같으면 간단히 하드코딩해서 보여주고 협의하고 작업할걸 그랬네. (…) 아무리 고쳐도 별로라서 버릴 코드라면 지금 작업이 너무 낭비되는 시간들이 된다."* 시각적 결과물은 합의 시점이 늦을수록 폐기 비용이 커진다.
...
**How to apply:**
- 이 파일은 git 동기화가 안 되므로 **머신마다 1회** 넣어야 한다 (§2B.2 방식 C 의 settings.json 등록과 같은 성격).
- 이 규칙이 로드된 세션에서 `~/.claude/settings.json` 에 이 키가 없으면 Claude 가 넣는다. 다른 키는 건드리지 않고 이 한 줄만 추가하며, 인코딩·BOM·한글 경로 보존은 §2B.3 을 따른다.
- 껐더라도 남는 부수 규칙 두 가지:
  - 사용자가 *"아까 ~하라고 했잖아"* 라고 하는데 Claude 기록에 그 문장이 없으면, **사용자의 기억을 틀렸다고 단정하지 말 것.** 화면에 표시된 문구인지 먼저 확인한다. "제 기록에는 없습니다" 까지가 Claude 가 말할 수 있는 전부다.
  - 설계·범위를 바꾸는 요청이 들어오면, Claude 가 먼저 제안한 것이 아닌 한 **착수 전에 한 줄로 타당성 판단을 밝히고** 진행한다 ("진행할까요?" 를 묻는 것이 아니다 — §6 과 충돌하지 않는다).

**Why:** 두 가지 문제가 겹친다.

1. **제안 문구는 모델 컨텍스트에 들어오지 않는다.** 화면에서는 Claude 응답 바로 아래 이어져 보이지만 Claude 는 그 문장을 못 본다. 사용자가 그것을 수락하지 않고 *되물으면* Claude 는 기록에서 찾지 못해 자기 말이 아니라고 부정한다. 2026-08-28 SCDeskTools 세션에서 실제로 발생 — Claude 가 *"쓴 적이 없습니다. 다른 세션의 기억이시거나 잘못 읽으신 것 같습니다"* 로 사용자의 기억을 부정했고, 사용자가 화면 캡처를 제시한 뒤에야 CLI 기능임이 확인됐다.
2. **책임의 고리가 끊긴다.** 제안은 Claude 의 판단을 거치지 않고 생성되는데 사용자에게는 Claude 의 권고로 읽힌다. 사용자가 수락하면 그것이 *사용자 명령* 이 되고, Claude 는 사용자 명령이므로 (§6 재확인 금지에 따라) 검토 없이 수행한다. 결과적으로 그 작업의 타당성을 **아무도 검토하지 않는다.**

사용자 명시(2026-08-28) — *"어떨때는 도움이 되었으나 도움되지 않거나 상황에 맞지 않는 속터지는 제안을 하는 바람에 내가 기분이 더 상할때가 많았다. 끄도록 하자."*
