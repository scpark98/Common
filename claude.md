...
## 1A. 미디어 파일명·개인 경로 — 기록에 남기지 말 것 (강제, 최우선)

소스 코드 / commit message / commit diff / README / 주석 / 로그 / TODO — **어떤 위치에도** 사용자의 *실제 미디어 파일명* 이나 *개인 경로* 를 기록하지 말 것.

**X — 절대 금지:**
- `open_media(_T("E:\\<폴더>\\<실제 작품 제목>.mkv"), ...)` 같은 hardcoded 테스트 path.
- commit message subject / body 에 `"<실제 미디어 제목> 한일자막 통합"` 같이 작품명 포함.
- README / claude.md / TODO 의 작업 항목에 검증용 미디어를 *실제 제목* 으로 기록.
- 진단 로그 (`logWrite`, `gLog` 등) literal 인자에 실제 path.
- 주석 안 (`//test with E:\...`) 에도 금지.

**O — 정공법:**
- 테스트 미디어 path 는 *registry 옵션* / *환경변수* / *런타임 인자* 로 받기. 코드엔 placeholder 또는 변수 reference 만.
- commit / README / TODO 작업 항목은 *동작·기능 카테고리* 만 (예: "다국어 자막 멀티 클래스 머지" — *어느 작품으로 검증* 했는지는 안 씀).
- 로그에 path 가 필요하면 basename hash 또는 마지막 N글자 fragment 만 (예: `"…<hash:a1b2c3>"`).
- `sample.mp4`, `test.mkv` 같은 일반 placeholder 는 OK.
...
**의심되면 멈추고 묻기 (강제):** 새 테스트 코드 / commit message / 로그 literal / README 항목 작성 직전 자문 — *이게 사용자의 시청 취향·개인 디렉토리를 식별 가능하게 하나?* 조금이라도 그럴 듯하면 **작성 전** 사용자에게 확인. 임의로 *"이 정도면 괜찮겠지"* 판단 절대 금지.

**적용 범위 — "기록" 의 정의:** 소스 코드 (`.cpp/.h/.py/.md/.rc/.txt/...`), commit subject / body, commit diff, README, claude.md, 주석, 진단 로그의 literal 인자, TODO / FIXME 메모, PR description, issue comment. 모두 포함.
...
4. **Common 작업 끝마다 즉시 commit + push** — 어떤 프로젝트 작업 중 Common 을 수정했으면 그 작업 단위 마무리 시점에 *Common 도 함께* push. 다른 프로젝트 세션이 같은 파일 건드릴 가능성과 머신 간 sync 누락 둘 다 줄어든다.

5. **다른 세션의 작업분 sync push** — 본인 (현재 세션) 이 수정하지 않은 변경이 git status 에 있으면 *별도 commit* 으로 분리하고 본문 끝에 `(타 프로젝트 세션 작업분 — 이 commit 은 sync 목적.)` 같은 한 줄 명시. 메시지 내용이 부정확할 수 있음을 인정.
