...
## 2F.2 Claude 가 작성/수정하는 주석에는 `(by claude)` 태그 (강제)

Claude 가 **새로 작성하거나 실질적으로 다시 쓰는 주석**에는 반드시 `(by claude)` 문구를 붙인다. 코드가 아니라 **주석 한정**이다.

- **한 줄 주석**: 끝에 붙인다 — `//SetRedraw(FALSE) 로 native 무효화 paint 를 묶는다 (by claude)`
- **여러 줄 주석 블록**: 블록당 **한 번**, 첫 줄 끝(또는 마지막 줄 끝)에 붙인다. 매 줄마다 붙이지 않는다.
- **기존 사용자 주석을 Claude 가 실질적으로 고쳐 쓰면** 그것도 `(by claude)` 로 표시(원저자와 구분). 오탈자·띄어쓰기 등 사소한 수정은 예외.
- 커밋 메시지·PR 본문·문서(`.md`) 본문에는 붙이지 않는다 — **소스 코드 주석에만** 적용(커밋은 이미 `Co-Authored-By` 로 표시됨).

**Why:** 이 사용자의 모든 repo 는 git committer 가 `scpark98` 단일 ID 라, `git blame` 으로는 그 주석을 *사용자가* 썼는지 *Claude 가* 썼는지 구분할 수 없다. 실제로 2026-07-04, `CVtListCtrlEx::OnNMRClick` 위의 잘못된 3줄 주석(2026-07-02 `84661721`, Claude 작성)을 두고 사용자가 *"이 주석은 내가 작성한듯한데... 이해가 안 된다"* 며 자기 것으로 오인했다. 문구 최초 등장 커밋을 추적해서야 Claude 작성임이 판명됐다. `(by claude)` 태그가 있었다면 즉시 구분됐을 것 — 사용자 명시 규칙화(2026-07-04): *"클로드가 주석을 작성할때는 반드시 'by claude'라는 문구를 꼭 붙이고 작성하라."*

**How to apply:** 코드에 주석을 추가·수정하는 매 순간 자문 — *이 주석은 내가(Claude) 쓰는가?* 그렇다면 `(by claude)` 를 포함해 작성. §2F(왜만 작성)·§2F.1(계약은 .h) 와 함께 적용되며, 태그는 그 규칙들 위에 얹는 표시일 뿐 주석 내용 규칙을 바꾸지 않는다.
