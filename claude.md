...
> (d) **사용자가 실제로 쓰는 구성과 다른 구성으로 빌드하기 일쑤다.** VS 의 활성 구성은 `.suo`(바이너리)에
> 있어 Claude 가 쉽게 알 수 없고, `.vcxproj` 에는 안 쓰는 구성이 잔재로 남아 있다(§5-1). 그래서
> 엉뚱한 구성으로 빌드해 그 구성에서만 나는 에러를 원인이라 착각하고 시간을 쓴다
