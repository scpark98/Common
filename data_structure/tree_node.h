...
public:
	...
	/// Return iterator to the parent of a node.
	//template<typename	iter> static iter get_next(iter);
...
//수직으로 하위 아이템을 찾아 리턴한다.
//하위는 sibling 또는 child인데 만약 없다면
//위로 올라가서 다음 sibling을 찾는다.
/*
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::get_next(iter it) 
{
	assert(it.node!=0);
