...
///////////////////////////////////////////////////////
//construction destruction
CDelayedDirectoryChangeHandler::CDelayedDirectoryChangeHandler(HWND parent, CDirectoryChangeHandler * pRealHandler, bool bAppHasGUI, LPCTSTR szIncludeFilter, LPCTSTR szExcludeFilter, DWORD dwFilterFlags)
...
//			The client is notified by calling CDirectoryChangeHandler's virtual functions On_FileAdded(),On_FileRemoved(), etc.
{
	...
	{
		TRACE(_T("%d, %s, %s\n"), pNot->m_eFunctionToDispatch, pNot->m_szFileName1, pNot->m_szFileName2);
		::SendMessage(m_parent, Message_CDirectoryChangeWatcher,
					(WPARAM)&CDirectoryChangeWatcherMessage(pNot->m_eFunctionToDispatch, pNot->m_szFileName1, pNot->m_szFileName2), 0);

		if(m_pRealHandler && m_pRealHandler->On_FilterNotification(pNot->m_eFunctionToDispatch,
											  pNot->m_szFileName1,
											  pNot->m_szFileName2) )
		{
			return true;
