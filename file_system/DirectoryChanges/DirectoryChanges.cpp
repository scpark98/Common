									...
									)
{
	...
	if( strDirToWatch.IsEmpty() || dwChangesToWatchFor == 0)
	//||  pChangeHandler == NULL )
	{
	...
	{
		VERIFY(UnwatchDirectory(strDirToWatch));
	}
	//
	//
	...
	CDirWatchInfo* pDirInfo = new CDirWatchInfo(hDir, strDirToWatch, parent, dwChangesToWatchFor, bWatchSubDirs, pChangeHandler, m_bAppHasGUI, szIncludeFilter, szExcludeFilter, m_dwFilterFlags);
	if(!pDirInfo)
	{
...
{ 
	...
	//
	//
	m_pChangeHandler = new CDelayedDirectoryChangeHandler(parent, pChangeHandler, bAppHasGUI, szIncludeFilter, szExcludeFilter, dwFilterFlags );
	if( m_pChangeHandler ) 
