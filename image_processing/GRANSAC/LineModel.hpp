...
public:
	...
	Point2D(GRANSAC::VPFloat xx, GRANSAC::VPFloat yy)
	{
		x = xx;
		y = yy;
	};
...
protected:
	...
	virtual GRANSAC::VPFloat ComputeDistanceMeasure(std::shared_ptr<GRANSAC::AbstractParameter> Param) override
	{
		...
		GRANSAC::VPFloat Numer = fabs(m_a * ExtPoint2D->x + m_b * ExtPoint2D->y + m_c);
		GRANSAC::VPFloat Dist = Numer / m_DistDenominator;

		// // Debug
		// std::cout << "Point: " << ExtPoint2D->x << ", " << ExtPoint2D->y << std::endl;
...
public:
	...
	virtual void Initialize(std::vector<std::shared_ptr<GRANSAC::AbstractParameter>> InputParams) override
	{
		...
		// m_d = Point2->y - m_m * Point2->x; // Intercept - alternative should be the same as above
