...
static int next_row(struct stream_state* stream)
{
	...
	memcpy( (uint8_t*)(stream->row), (uint8_t*)(stream->data) + stream->y * stream->w, stream->w );

	if ( stream->threshold < 0 )
	{
		return 0;
	}
	for (int x = 0; x < stream->w; x++)
	{
		stream->row[x] = (stream->row[x] >= stream->threshold) * 255;
	}

    return 0;
}
/*
static int next_frame(struct stream_state* stream)
{
    stream->wrap = 0;
    stream->x = 0;
    stream->y = -1;
    return next_frame_hook(stream);
}
*/
...
static void flush_incremental(struct blob_list* blist, struct blob* blob_now)
...
static void flush_old_blobs(struct blob_list* blist, int y)
...
// center_x, center_y and size are the cumulative stats for a blob
{
	...
	vt_result.push_back( CBlobs(b->bb_x1, b->bb_y1, b->bb_x2, b->bb_y2, b->center_x, b->center_y, b->size, b->color) );
	//printf("%i, %i, %i, %i\n", b->bb_x1, b->bb_y1, b->bb_x2, b->bb_y2);
	//if (0)//options->show_bb)
	//{
...
bool quick_blobs( std::vector<CBlobs> &blobs, uint8_t *data, int w, int h, uint8_t threshold )
{
	...
	stream.row = (uint8_t*)malloc( stream.w );
	stream.x = 0;
	...
	//while (!next_frame(&stream))
	{
		init_blobs(&blist);
		while (!next_row(&stream))
		{
			blob_prev = blist.head->next;
			while (!stream.wrap)
			{
				...
				sib_find(blist.head->next, blob_now);
				blob_insert(blob_prev, blob_now);
				flush_incremental(&blist, blob_now);
				blob_prev = blob_now;
			}
			flush_old_blobs(&blist, stream.y);
			//show_status(blist.head, &stream);
			//show_dead_sibs(blist.head);
			//show_blobs(blist.head);
			//printf("----------\n");
		}
		flush_old_blobs(&blist, stream.h - 1);
	}

	//close_pixel_stream(&stream);
	if ( stream.row )
	{
		free(stream.row);
		stream.row = NULL;
	...
	blobs.clear();
	blobs.assign( vt_result.begin(), vt_result.end() );
	return true;
}
