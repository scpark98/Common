...
#ifndef xy_vector
typedef struct { double x, y; } xy_vector; 
#endif
...
#define PAR_DEFAULT_NSCALES 1//10
	...
	)
{
	...
	float I2w[INIT_SIZE];
	float I2wy[INIT_SIZE];
	float I2wx[INIT_SIZE];
	float I2xx[INIT_SIZE];
	float I2yy[INIT_SIZE];
	float I2xy[INIT_SIZE];
	float I2wxx[INIT_SIZE];
	float I2wyy[INIT_SIZE];
	float I2wxy[INIT_SIZE];

	float div_u[INIT_SIZE];
	float div_v[INIT_SIZE];
	float div_d[INIT_SIZE];
	...
	float psid[INIT_SIZE];
	float psig[INIT_SIZE];
	float psis[INIT_SIZE];
	float psi1[INIT_SIZE];
	float psi2[INIT_SIZE];
	float psi3[INIT_SIZE];
	float psi4[INIT_SIZE];
	...
	)
{
	...
	float I1s[PAR_DEFAULT_NSCALES][INIT_SIZE];
	float I2s[PAR_DEFAULT_NSCALES][INIT_SIZE];
	float us[PAR_DEFAULT_NSCALES][INIT_SIZE];
	float vs[PAR_DEFAULT_NSCALES][INIT_SIZE];
	//std::vector<float *> us (nscales);
	//std::vector<float *> vs (nscales);
	int nx[PAR_DEFAULT_NSCALES];
	int ny[PAR_DEFAULT_NSCALES];
...
//		vs[s]  = new float[sizes];
	...
	{
		//if(1) std::cout << "Scale: " << s << std::endl;
...
xy_vector optical_flow( unsigned char *img1, unsigned char *img2, int w, int h)
{
	...
	const float N = 1 + log(std::min(21, 21) / 16.) / log(1./0.7);
	//if((int) N < nscales) nscales = (int) N;

	int i;
	xy_vector vector;

	float *I1 = (float*) malloc(sizeof(float) * w * h);
	for ( i = 0; i < w*h; i++ )
		I1[i] = (float)img1[i];

	float *I2 = (float*) malloc(sizeof(float) * w * h);
	for ( i = 0; i < w*h; i++ )
		I2[i] = (float)img2[i];

	//allocate memory for the flow
	float *u = (float*) malloc(sizeof(float) * w * h);
