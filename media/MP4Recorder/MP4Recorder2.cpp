...
extern char* WCHARToUTF8(const WCHAR* pwstrSrc);

CMP4Recorder2::CMP4Recorder2()
:	m_hMP4File(NULL)
,	m_nVTrackId(0)
,	m_nATrackId(0)
...
bool CMP4Recorder2::Start(CString file, int width, int height, int fps, int quality)
{
...
bool CMP4Recorder2::Start(CMP4RecorderParam* param)
{
	bool bResult = true;
...
void CMP4Recorder2::Pause()
{
	m_bPauseRecord = true;
}

void CMP4Recorder2::Resume()
{
	m_dwLastVideoTimestamp = m_dwLastAudioTimestamp = timeGetTime();
	m_bPauseRecord = false;
}

void CMP4Recorder2::WriteVideoSample(BYTE* lpData, int nSize, int nCapWidth, int nCapHeight, double fTimeStamp)
{
