...
{
	XMSGBOXPARAMS param;
	param.nTimeoutSeconds = nTimeOut;
	return XMessageBox( hwnd, lpszMessage, lpszCaption, MB_OK | MB_ICONINFORMATION | nStyle, &param, pWndMsgBox );
}
	...
	m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)				//+++1.8
{
	...
	for (int button_index = 0; button_index < LAST_BUTTON; button_index++)
	{
		...
		//scpark
		//아래 코드는 원래 기본 버튼의 너비를 늘려서 그 버튼의 캡션에 타임아웃 시간을 표시했으나
		//타이틀바에서 표시하도록 하기 위해 주석처리 하였다. WM_TIMER에서 타이틀바를 변경시켜준다.
		...
		case WM_TIMER:		// used for timeout
		{
			if (wParam == 1)			// timeout timer
			{
				...
				*/
				...
				_stprintf( szTimeoutCaption, _T("%s (%d초 후 닫힘)"), Me->m_lpszCaption, Me->m_nTimeoutSeconds );
