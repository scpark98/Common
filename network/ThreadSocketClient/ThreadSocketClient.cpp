#include "StdAfx.h"
#include "ThreadSocketClient.h"

/*
SocketThreadCom : 소켓 recv 쓰레드
용도 : 쓰레드로 돌면서 수신과 연결상태를 체크. 수신이나 연결상태가 변하면
DualSocket::m_psocket_func 의 함수 포인터로 콜
*/
static UINT SocketThreadCom(LPVOID dParameter)
{
	CThreadSocketClient *plpr_socket = (CThreadSocketClient*)dParameter;
...
static UINT ConnectingThreadCom(LPVOID dParameter)
{
	...
	while(plpr_socket->m_con_state == 1 && myp == plpr_socket->m_connect_thread)
	{
		...
		if(connect(plpr_socket->m_socket, (sockaddr *)&addr, sizeof(addr)) == SOCKET_ERROR)
		{
			...
			plpr_socket->m_psocket_func(FD_CONNECT, (char*)CThreadSocketClient::FAILTOCONNECT);
		...
		plpr_socket->m_con_state = CThreadSocketClient::CONNECTED;
		TRACE("Connection success.\n");

		plpr_socket->m_psocket_func(FD_CONNECT, (char*)CThreadSocketClient::CONNECTED);
...
CThreadSocketClient::CThreadSocketClient(void)
{	
	m_con_state = 0;
	m_psocket_func = NULL;
...
int CThreadSocketClient::Init(void (*p_func)(int, char*))
{
	WSADATA wsadata = {0};
...
*/
int CThreadSocketClient::Connect(char *ip, int port, bool auto_recon, int recon_ms /* = 5000*/)
{
...
Disconnect : 연결 해제
*/
int CThreadSocketClient::Disconnect()
{
	m_con_state = 0;
	m_auto_recon = false;
	if(m_socket != INVALID_SOCKET)
...
*/
int CThreadSocketClient::Send(char *str)
{
	if(m_socket == INVALID_SOCKET)
	{
