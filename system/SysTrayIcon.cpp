...
LRESULT CSysTrayWnd::OnSysTrayMsg(WPARAM wParam, LPARAM lParam)
{
	...
	::SendMessage(m_pstiSysTrayIcon->GetParent(), WM_SYSTRAYMSG, wParam, lParam);
	return 0;
...
BOOL CSysTrayIcon::CreateIcon(HICON hIcon, UINT nIconID, LPCTSTR szTip)
{
	...
	if (bIconCreated == FALSE)
	{
		...
		if (pnidIconInfo != NULL)
		{
			...
			ASSERT(hIcon != NULL);
			ASSERT(_tcscmp(szTip, _T("")) != 0); // Sure that icon tip == "" ??
			...
			if (hIcon != NULL)
			{
				...
				_tcscpy(pnidIconInfo->szTip, szTip); // Copy the string
...
BOOL CSysTrayIcon::ShowBalloon(UINT  nIconID, LPCTSTR szBalloonTitle, LPCTSTR szBalloonMsg, DWORD dwIcon, UINT nTimeOut)
{
	...
	ASSERT(_tcscmp(szBalloonTitle, _T("")) != 0); // Sure that balloon title == "" ??
	ASSERT(_tcscmp(szBalloonMsg, _T("")) != 0); // Sure that balloon message == "" ?
	...
	_tcscpy(pnidIconInfo->szInfoTitle, szBalloonTitle);

	// Set balloon message
	_tcscpy(pnidIconInfo->szInfo, szBalloonMsg);
...
/*-----------------------------------------------------------------------------
 Function : CSysTrayIcon::OnMouseMove()
...
BOOL CSysTrayIcon::ChangeIconTip(UINT nIconID, CString strTip)
{
	...
	if (pnidIconInfo != NULL)
	{
		_tcscpy(pnidIconInfo->szTip, szTip); // Copy the new string containing the tip
