...
/*
* 20241126 scpark 도입.
* SystemTray.h, cpp 두 파일을 프로젝트에 추가하고 cpp 파일의 속성에서 "미리 컴파일된 헤더 사용 안 함"으로 변경.
* main에서 다음과 같이 인스턴스를 선언하고
    CSystemTray		m_tray;
...
public:
    ...
    // For icon animation
    //시작과 끝 아이콘 id를 전달한다. 두 아이콘 및 그 사이에도 아이콘들이 모두 존재해야 한다.
    bool    set_icon_list_range(UINT icon_first, UINT icon_last);

    //아이콘들을 나열하여 추가한다.
    //m_tray.set_icon_list(IDI_ICON_00, IDI_ICON_01, IDI_ICON_02);
    template <typename ... Types> void set_icon_list(Types... icon_ids)
    {
        m_iconlist.clear();

        int n = sizeof...(icon_ids);
        if (n == 0)
            return;

        UINT arg[] = { icon_ids... };
        for (auto icon_id : arg)
        {
            m_iconlist.push_back(AfxGetApp()->LoadIcon(icon_id));
        }
    }

    bool    is_animating() { return (m_uIDTimer > 0); }
...
protected:
    ...
    std::deque<HICON> m_iconlist;
    UINT_PTR     m_uIDTimer = 0;
